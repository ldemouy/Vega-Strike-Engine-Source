...
{
  struct category *OLD = &CATS;
  struct category *NEW = &CATS;
  do
  {
    if (!OLD->name)
    {
    ...
    if (strcmp(OLD->group, CURRENT->name) != 0)
    {
      continue;
    }
    if (strcmp(OLD->name, CURRENT->setting) == 0)
      break;
  ...
  do
  {
    ...
    if (strcmp(NEW->group, CURRENT->name) != 0)
    {
      continue;
    }
    if (strcmp(GetInfo(NEW->name), selectedstr) == 0)
      break;
...
void ShowMain()
{
  ...
  do
  {
    ...
    do
    {
      ...
      if (strcmp(CURRENT->name, CUR->group) != 0)
      {
        continue;
      }
      i++;
  ...
  while (true)
  {
    ...
    do
    {
      ...
      if (strcmp(CURRENT->name, selectedstr) == 0)
        break;
      ++selected;
    } while ((CURRENT = CURRENT->next) > 0);
    CUR = &CATS;
