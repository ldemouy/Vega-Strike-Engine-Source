...
NewButton::NewButton(void)
    : m_drawingState(NewButton::NORMAL_STATE), m_commandId(), m_leftPressed(false),
      m_highlightColor(GUI_OPAQUE_WHITE()), m_textHighlightColor(GUI_CLEAR), m_downColor(GUI_CLEAR),
      m_downTextColor(GUI_CLEAR), m_shadowWidth(1.0), m_variableBorderCycleTime(0.0), m_borderColor(GUI_OPAQUE_BLACK()),
      m_endBorderColor(GUI_OPAQUE_WHITE()), m_currentCycleColor(GUI_OPAQUE_BLACK()), m_currentCycle(0),
      m_cycleStepCount(-1), m_cycleDirection(0), m_cycleColorDelta(), m_lastStepTime(0.0)
{
}
