...
void PaintText::calcLayout(void)
{
    ...
    if (m_widthExceeded != MULTI_LINE)
    {
        ...
        currentLine->baseLine = (currentLine->height - m_font.size()) / 2.0 + m_verticalScaling * REFERENCE_FONT_ASCENDER;
...
//CONSTRUCTION
PaintText::PaintText(void) : m_rect(), m_text(), m_color(GUI_OPAQUE_BLACK()), m_font(), m_justification(RIGHT_JUSTIFY), m_widthExceeded(ELLIPSIS), m_needLayout(true), m_layoutVersion(0), m_verticalScaling(0.7), m_horizontalScaling(0.7)
{
}

PaintText::PaintText(const Rect &r, const std::string &t, const Font &f, const GFXColor &c, Justification j,
                     WidthExceeded w) : m_rect(r), m_text(), //Don't set text here.
                                        m_color(c), m_font(f), m_justification(j), m_widthExceeded(w), m_needLayout(true), m_layoutVersion(0), m_verticalScaling(0.7), m_horizontalScaling(0.7)
{
