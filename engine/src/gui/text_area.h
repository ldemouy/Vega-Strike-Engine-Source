...
// Changes:
// 1.1 - Fixed misaligned down scroll button (FINALLY). Fixed text going past the text area (horizontal). Added
// multiline text areas. 1.2 - Added sorting 1.3 - Added DoMouse to classes. Also fixed bug where button class would
// always 1 for dragging
...
// There are places where a float is converted to an int. This define takes extra steps to convert without a warning
// (float -> char -> int)
#define NO_WARNINGS
...
class TextArea
{
  public:
    ...
    void AddTextItem(const char *name, const char *description, const char *parent_name = nullptr,
    ...
    // The class requires you convert the x,y co-ordinate to a -1 to 1 float. The class doesn't know the screen
    // dimensions
  ...
  private:
    ...
    // 0 for no button clicked, 1 for top button, 2 for bottom button, 3 for scrollbar, 4 for above scrollbar, 5 for
    // below scrollbar
    char button_pressed;
