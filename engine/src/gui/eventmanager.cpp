...
{
    if (controlToDelete == nullptr ||
        find(deleteQueue.begin(), deleteQueue.end(), controlToDelete) != deleteQueue.end())
    {
...
)
{
    ...
    // FIXME -- Calling this here causes the loop in processEvent to be b0rked by a deletion. We now must
    // checkForShutdown at a later time...which is below    checkForShutDownEventManager();     // FIXME mbyron -- EVENT
    // HACK.
}
...
EventManager::EventManager(void) : m_mouseLoc(0.0, 0.0)
{
}
