...
bool useStroke()
{
    static bool tmp = XMLSupport::parse_bool(vs_config->getVariable(
        "graphics", "high_quality_font_computer", vs_config->getVariable("graphics", "high_quality_font", "false")));
    return !tmp;
}
...
void Font::calcMetrics(void)
{
    ...
    const double nonClippedStrokeWidth =
        size() * referenceStrokeWidth * strokeWeight() * (g_game.x_resolution / referenceStrokeWidthResolution);
...
double Font::charWidth(char c) const
{
    ...
    if (useStroke())
    {
        if (c == SPACE_CHAR)
        {
            ...
            const double spaceCharWidth = glutStrokeWidth(GLUT_STROKE_ROMAN, SPACE_CHAR) + m_spaceCharFixup;
