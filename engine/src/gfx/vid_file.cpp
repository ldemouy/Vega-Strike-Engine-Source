...
//Decode the next chunk of data
                    ...
                    //Was there an error?
                    if (bytesDecoded <= 0)
                        throw VidFile::FrameDecodeException("Error decoding frame");
//Crappy ffmpeg!
#if (LIBAVCODEC_VERSION_MAJOR >= 53)
...
public:
    ...
    VidFileImpl(size_t maxDimensions, bool forcePOT) : pFormatCtx(0), pCodecCtx(0), pCodec(0), pFrameRGB(0), pFrameYUV(0), pNextFrameYUV(0), pStream(0), frameReady(false), packetBuffer(0), packetBufferSize(0), fbDimensionLimit(maxDimensions), fbForcePOT(forcePOT), frameBuffer(0)
    {
        packet.data = 0;
    }
    ...
    {
        if (pCodecCtx != 0)
            throw VidFile::Exception("Already open");
        ...
        if ((0 != avformat_open_input(&pFormatCtx, npath.c_str(), NULL, NULL)) || (0 > av_find_stream_info(pFormatCtx)))
            throw VidFile::FileOpenException(errbase + " (wrong format or)");
//Dump format info in case we want to know...
        ...
        if (pCodecCtx == 0)
            throw VidFile::FileOpenException(errbase + " (no video stream)");
        ...
        if (pCodec == 0)
            throw VidFile::UnsupportedCodecException(errbase + " (unsupported codec)");
        ...
        {
            width = height = 1;
            while (width < pCodecCtx->width && width <= (fbDimensionLimit / 2))
                width *= 2;
            while (height < pCodecCtx->height && height <= (fbDimensionLimit / 2))
                height *= 2;
        }
        else
        {
        ...
        if (pFrameRGB == 0)
            throw VidFile::Exception("Problem during RGB framebuffer initialization");
