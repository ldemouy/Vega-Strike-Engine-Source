...
template <typename T> T _netswap(T t)
{
    return t;
}

template <> float _netswap(float f)
{
    return VSSwapHostFloatToLittle(f);
}

template <> double _netswap(double d)
{
    return VSSwapHostDoubleToLittle(d);
}

template <typename T> T _sqrt(T t)
{
    return t;
}

template <> float _sqrt(float f)
{
    return sqrtf(f);
}

template <> double _sqrt(double d)
{
    return sqrtf(d);
}

// Constructors
template <typename S, typename T> TVector<S, T>::TVector(const TVector<T, S> &a)
{
    i = static_cast<S>(a.i);
    j = static_cast<S>(a.j);
    k = static_cast<S>(a.k);
...
template <typename S, typename T> TVector<S, T>::TVector(S i, S j, S k)
{
    this->i = i;
    this->j = j;
    this->k = k;
...
template <> TVector<float, double>::TVector(PyObject *p)
{
...
template <> TVector<double, float>::TVector(PyObject *p)
{
...
// Operators
template <typename S, typename T> TVector<S, T> TVector<S, T>::operator+(const TVector<S, T> &obj) const
{
...
template <typename S, typename T> S TVector<S, T>::operator*(const TVector &b) const
{
...
template <typename S, typename T> TVector<S, T> &TVector<S, T>::operator=(const TVector<S, T> &other)
{
    this->i = other.i;
    this->j = other.j;
    this->k = other.k;
...
template <typename S, typename T> const TVector<T, S> &TVector<S, T>::operator=(const TVector<T, S> &a)
{
    i = static_cast<S>(a.i);
    j = static_cast<S>(a.j);
    k = static_cast<S>(a.k);
...
template <typename S, typename T> TVector<S, T> TVector<S, T>::operator-() const
{
    return TVector(-i, -j, -k);
}

template <typename S, typename T> bool TVector<S, T>::operator==(const TVector<S, T> &b) const
{
    return i == b.i && j == b.j && k == b.k;
}

template <typename S, typename T> TVector<S, T> &TVector<S, T>::operator+=(const TVector<S, T> &obj)
{
    i += obj.i;
    j += obj.j;
    k += obj.k;
...
template <typename S, typename T> TVector<S, T> &TVector<S, T>::operator*=(const T &obj)
{
    i *= obj;
    j *= obj;
    k *= obj;
...
template <typename S, typename T> TVector<T, S> TVector<S, T>::Cast() const
{
    return TVector<T, S>(i, j, k);
}

template <typename S, typename T> TVector<S, T> TVector<S, T>::Cross(const TVector<S, T> &v) const
{
    return TVector(this->j * v.k - this->k * v.j, this->k * v.i - this->i * v.k, this->i * v.j - this->j * v.i);
}

template <typename S, typename T> S TVector<S, T>::Dot(const TVector &b) const
{
    return DotProduct(*this, b);
}

// Not used. Specialized below.
template <typename S, typename T> S TVector<S, T>::Magnitude() const
{
    return 1;
}

template <> float TVector<float, double>::Magnitude() const
{
    return sqrtf(i * i + j * j + k * k);
}

template <> double TVector<double, float>::Magnitude() const
{
    return sqrt(i * i + j * j + k * k);
}

template <typename S, typename T> S TVector<S, T>::MagnitudeSquared() const
{
    return i * i + j * j + k * k;
}

template <typename S, typename T> void TVector<S, T>::netswap()
{
...
template <typename S, typename T> TVector<S, T> TVector<S, T>::Normalize()
{
    ::Normalize(*this);
    return *this;
}

template <typename S, typename T> TVector<S, T> TVector<S, T>::Scale(S s) const
{
    return TVector(s * i, s * j, s * k);
}

template <typename S, typename T> void TVector<S, T>::Set(S x, S y, S z)
{
    i = x;
    j = y;
    k = z;
...
template <typename S, typename T> TVector<S, T> TVector<S, T>::Vabs() const
{
    return TVector(i >= 0 ? i : -i, j >= 0 ? j : -j, k >= 0 ? k : -k);
}

template <typename S, typename T> TVector<S, T> TVector<S, T>::Min(const TVector<S, T> &other) const
{
    return TVector((i < other.i) ? i : other.i, (j < other.j) ? j : other.j, (k < other.k) ? k : other.k);
}

template <typename S, typename T> TVector<S, T> TVector<S, T>::Max(const TVector<S, T> &other) const
{
    return TVector((i > other.i) ? i : other.i, (j > other.j) ? j : other.j, (k > other.k) ? k : other.k);
}
...
template <typename S, typename T> void TVector<S, T>::Yaw(S rad) // only works with unit vector
{
    float theta;
...
template <typename S, typename T> void TVector<S, T>::Roll(S rad)
{
    float theta;
    theta = 0.0f; // FIXME This line added temporarily by chuck_starchaser
    if (i > 0)
...
template <typename S, typename T> void TVector<S, T>::Pitch(S rad)
{
    float theta;
    theta = 0.0f; // FIXME This line added temporarily by chuck_starchaser
    if (k > 0)
...
template <> const TVector<double, float> TVector<double, float>::Transform(const Matrix &m1) const
{
...
template <> const TVector<float, double> TVector<float, double>::Transform(const Matrix &m1) const
{
...
{
    return TVector<S, T>(p.i * v.i + q.i * v.j + r.i * v.k, p.j * v.i + q.j * v.j + r.j * v.k,
                         p.k * v.i + q.k * v.j + r.k * v.k);
}

// Template Instantiation
template class TVector<float, double>;
template class TVector<double, float>;
...
template void CrossProduct(const TVector<float, double> &a, const TVector<float, double> &b,
                           TVector<float, double> &RES);
template void CrossProduct(const TVector<double, float> &a, const TVector<double, float> &b,
                           TVector<double, float> &RES);

template void ScaledCrossProduct(const TVector<float, double> &a, const TVector<float, double> &b,
                                 TVector<float, double> &r);
template void ScaledCrossProduct(const TVector<double, float> &a, const TVector<double, float> &b,
...
template TVector<float, double> PolygonNormal(TVector<float, double> v1, TVector<float, double> v2,
                                              TVector<float, double> v3);
template TVector<double, float> PolygonNormal(TVector<double, float> v1, TVector<double, float> v2,
...
template TVector<float, double> Transform(TVector<float, double> p, TVector<float, double> q, TVector<float, double> r,
                                          TVector<float, double> v);
template TVector<double, float> Transform(TVector<double, float> p, TVector<double, float> q, TVector<double, float> r,
