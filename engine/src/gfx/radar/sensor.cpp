...
{
  public:
    CollectRadarTracks() : sensor(nullptr), player(nullptr), collection(nullptr)
    {
    }
...
// FIXME: Scale objects according to distance and ignore those below a given threshold (which improves with better
// sensors)
const Sensor::TrackCollection &Sensor::FindTracksInRange() const
{
    ...
    {
        findObjects(_Universe->activeStarSystem()->collidemap[Unit::UNIT_ONLY], player->location[Unit::UNIT_ONLY],
                    &unitLocator);
    }
    if (allGravUnits)
    {
        ...
        for (auto i = _Universe->activeStarSystem()->gravitationalUnits().constIterator(); (gravUnit = *i) != nullptr;
             ++i)
        {
