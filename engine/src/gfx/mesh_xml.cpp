...
const EnumMap::Pair MeshXML::element_names[] = {EnumMap::Pair("UNKNOWN", MeshXML::UNKNOWN),
...
const EnumMap::Pair MeshXML::attribute_names[] = {
    EnumMap::Pair("UNKNOWN", MeshXML::UNKNOWN), EnumMap::Pair("Scale", MeshXML::SCALE),
    EnumMap::Pair("Blend", MeshXML::BLENDMODE), EnumMap::Pair("alphatest", MeshXML::ALPHATEST),
    EnumMap::Pair("texture", MeshXML::TEXTURE), EnumMap::Pair("technique", MeshXML::TECHNIQUE),
    EnumMap::Pair("alphamap", MeshXML::ALPHAMAP), EnumMap::Pair("sharevertex", MeshXML::SHAREVERT),
    EnumMap::Pair("red", MeshXML::RED), EnumMap::Pair("green", MeshXML::GREEN), EnumMap::Pair("blue", MeshXML::BLUE),
    EnumMap::Pair("alpha", MeshXML::ALPHA), EnumMap::Pair("power", MeshXML::POWER),
    EnumMap::Pair("reflect", MeshXML::REFLECT), EnumMap::Pair("x", MeshXML::X), EnumMap::Pair("y", MeshXML::Y),
    EnumMap::Pair("z", MeshXML::Z), EnumMap::Pair("i", MeshXML::I), EnumMap::Pair("j", MeshXML::J),
    EnumMap::Pair("k", MeshXML::K), EnumMap::Pair("Shade", MeshXML::FLATSHADE), EnumMap::Pair("point", MeshXML::POINT),
    EnumMap::Pair("s", MeshXML::S), EnumMap::Pair("t", MeshXML::T),
    // Logo stuffs
    EnumMap::Pair("Type", MeshXML::TYPE), EnumMap::Pair("Rotate", MeshXML::ROTATE),
    EnumMap::Pair("Weight", MeshXML::WEIGHT), EnumMap::Pair("Size", MeshXML::SIZE),
    EnumMap::Pair("Offset", MeshXML::OFFSET), EnumMap::Pair("meshfile", MeshXML::LODFILE),
    EnumMap::Pair("Animation", MeshXML::ANIMATEDTEXTURE), EnumMap::Pair("Reverse", MeshXML::REVERSE),
    EnumMap::Pair("LightingOn", MeshXML::LIGHTINGON), EnumMap::Pair("CullFace", MeshXML::CULLFACE),
    EnumMap::Pair("ForceTexture", MeshXML::FORCETEXTURE), EnumMap::Pair("UseNormals", MeshXML::USENORMALS),
    EnumMap::Pair("UseTangents", MeshXML::USETANGENTS), EnumMap::Pair("PolygonOffset", MeshXML::POLYGONOFFSET),
    EnumMap::Pair("DetailTexture", MeshXML::DETAILTEXTURE), EnumMap::Pair("FramesPerSecond", MeshXML::FRAMESPERSECOND)};

const EnumMap MeshXML::element_map(MeshXML::element_names,
...
void Mesh::beginElement(MeshXML *xml, const string &name, const AttributeList &attributes)
{
    ...
    case MeshXML::DETAILPLANE:
        ...
        {
            Vector vec(detailPlanes.size() >= 2 ? 1 : 0, detailPlanes.size() == 1 ? 1 : 0,
    ...
    case MeshXML::MATERIAL:
        ...
        for (iter = attributes.begin(); iter != attributes.end(); iter++)
        {
            ...
            case MeshXML::LIGHTINGON:
                setLighting(XMLSupport::parse_bool(vs_config->getVariable("graphics", "ForceLighting", "true")) ||
    ...
    case MeshXML::LOCATION:
        ...
        assert((xml->point_state & (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z)) ==
               (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z));
        break;
    case MeshXML::NORMAL:
        ...
        if ((xml->point_state & (MeshXML::P_I | MeshXML::P_J | MeshXML::P_K)) !=
            (MeshXML::P_I | MeshXML::P_J | MeshXML::P_K))
        {
            if (!xml->recalc_norm)
            {
    ...
    case MeshXML::LINE:
        ...
        for (iter = attributes.begin(); iter != attributes.end(); iter++)
        {
            ...
            {
            case MeshXML::UNKNOWN:
                // VSFileSystem::vs_fprintf (stderr,"Unknown attribute '%s' encountered in Vertex
                // tag\n",(*iter).name.c_str() );
                break;
    ...
    case MeshXML::LOD:
        for (iter = attributes.begin(); iter != attributes.end(); iter++)
        {
            ...
            case MeshXML::LODFILE:
                xml->lod.push_back(
                    new Mesh((*iter).value.c_str(), xml->lodscale, xml->faction, xml->fg, true)); // make orig mesh
                break;
            case MeshXML::SIZE:
        ...
        break;
    case MeshXML::VERTEX: {
        assert(top == MeshXML::TRI || top == MeshXML::QUAD || top == MeshXML::LINE || top == MeshXML::TRISTRIP ||
               top == MeshXML::TRIFAN || top == MeshXML::QUADSTRIP || top == MeshXML::LINESTRIP);
        // assert(xml->load_stage==4);
        ...
        assert((xml->vertex_state & (MeshXML::V_POINT | MeshXML::V_S | MeshXML::V_T)) ==
               (MeshXML::V_POINT | MeshXML::V_S | MeshXML::V_T));
    ...
    case MeshXML::LOGO: {
        ...
        assert((xml->vertex_state & (MeshXML::V_TYPE | MeshXML::V_ROTATE | MeshXML::V_SIZE | MeshXML::V_OFFSET)) ==
               (MeshXML::V_TYPE | MeshXML::V_ROTATE | MeshXML::V_SIZE | MeshXML::V_OFFSET));
...
void Mesh::endElement(MeshXML *xml, const string &name)
{
    ...
    case MeshXML::UNKNOWN:
        ...
        break;
    case MeshXML::POINT:
        assert((xml->point_state &
                (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z | MeshXML::P_I | MeshXML::P_J | MeshXML::P_K)) ==
               (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z | MeshXML::P_I | MeshXML::P_J | MeshXML::P_K));
    ...
    case MeshXML::POINTS:
         ...
         *  clog << "Point: (" << xml->vertices[a].x << ", " << xml->vertices[a].y << ", " << xml->vertices[a].z << ")
         * (" << xml->vertices[a].i << ", " << xml->vertices[a].j << ", " << xml->vertices[a].k << ")\n";
         *  }
         *  clog << endl;
         */
        break;
                                ...
                                const std::vector<std::string> &overrideTextures)
{
    ...
    if (isbfxm || strstr(filename, ".obj"))
    {
        ...
        f.GoTo(0);
        hash_name = (err == VSFileSystem::Shared) ? VSFileSystem::GetSharedMeshHashName(filename, scale, faction)
...
{
    Vector v1(vertices[i2].x - vertices[i1].x, vertices[i2].y - vertices[i1].y, vertices[i2].z - vertices[i1].z);
    Vector v2(vertices[i3].x - vertices[i1].x, vertices[i3].y - vertices[i1].y, vertices[i3].z - vertices[i1].z);
    v1.Normalize();
    v2.Normalize();
...
static void SumNormals(vector<GFXVertex> &vertices, vector<int> &indices, size_t begin, size_t end, POLYTYPE polytype,
...
static float faceTSWeight(vector<GFXVertex> &vertices, int i1, int i2, int i3)
{
    ...
    Vector v1(vtx2.x - vtx1.x, vtx2.y - vtx1.y, vtx2.z - vtx1.z);
    Vector v2(vtx3.x - vtx1.x, vtx3.y - vtx1.y, vtx3.z - vtx1.z);
    v1.Normalize();
    v2.Normalize();
...
static void SumTangents(vector<GFXVertex> &vertices, vector<int> &indices, size_t begin, size_t end, POLYTYPE polytype,
                        vector<float> &weights)
{
    int flip = 0;
    size_t i;
...
void Mesh::PostProcessLoading(MeshXML *xml, const vector<string> &textureOverride)
{
    ...
    for (a = 0; a < xml->tris.size(); a += 3)
        for (j = 0; j < 3; j++)
        {
            ...
            xml->tris[a + j].SetTangent(xml->vertices[ix].GetTangent(), xml->vertices[ix].GetTangentParity());
        }
    a = 0;
    for (a = 0; a < xml->quads.size(); a += 4)
        for (j = 0; j < 4; j++)
        {
            ...
            xml->quads[a + j].SetTangent(xml->vertices[ix].GetTangent(), xml->vertices[ix].GetTangentParity());
        }
    a = 0;
    ...
    for (l = a = 0; a < xml->tristrips.size(); a++)
        for (k = 0; k < xml->tristrips[a].size(); k++, l++)
        {
            ...
            xml->tristrips[a][k].SetTangent(xml->vertices[ix].GetTangent(), xml->vertices[ix].GetTangentParity());
        }
    for (l = a = 0; a < xml->trifans.size(); a++)
        for (k = 0; k < xml->trifans[a].size(); k++, l++)
        {
            ...
            xml->trifans[a][k].SetTangent(xml->vertices[ix].GetTangent(), xml->vertices[ix].GetTangentParity());
        }
    for (l = a = 0; a < xml->quadstrips.size(); a++)
        for (k = 0; k < xml->quadstrips[a].size(); k++, l++)
        {
            ...
            xml->quadstrips[a][k].SetTangent(xml->vertices[ix].GetTangent(), xml->vertices[ix].GetTangentParity());
        }
    ...
    mx.j *= xml->scale.j;
    mx.k *= xml->scale.k;
    float x_center = (mn.i + mx.i) / 2.0, y_center = (mn.j + mx.j) / 2.0, z_center = (mn.k + mx.k) / 2.0;
    ...
    if (xml->sharevert)
    {
        vlist = new GFXVertexList((polytypes.size() ? &polytypes[0] : 0), xml->vertices.size(),
                                  (xml->vertices.size() ? &xml->vertices[0] : 0), o_index,
                                  (poly_offsets.size() ? &poly_offsets[0] : 0), false, (ind.size() ? &ind[0] : 0));
    }
    else
    {
        ...
        if (usopttmp && (vertexlist.size() > 0))
        {
            ...
            {
                vlist = new GFXVertexList((polytypes.size() ? &polytypes[0] : 0), numopt, newv, o_index,
                                          (poly_offsets.size() ? &poly_offsets[0] : 0), false, ind);
                cachunk = true;
            }
            free(ind);
            free(newv);
        ...
        if (!cachunk)
        {
            if (vertexlist.size() == 0)
                vertexlist.resize(1);
            vlist = new GFXVertexList((polytypes.size() ? &polytypes[0] : 0), totalvertexsize, &vertexlist[0], o_index,
