...
class Texture : public ::VSImage
{
  ...
  public:
    ...
    /// The minimum/maximum texturing coordinates for this texture (have in mind that it is not always enforce, ie. for
    /// meshes) (i=s, j=t, k=u)
  ...
  public:
    ...
    Texture(int stage = 0, enum FILTER mipmap = MIPMAP, enum TEXTURE_TARGET target = TEXTURE2D,
            enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D, enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE);

    /// Creates a texture with a single bitmap as color data and another grayscale .bmp as alpha data
    Texture(const char *, const char *, int stage = 0, enum FILTER mipmap = MIPMAP,
            enum TEXTURE_TARGET target = TEXTURE2D, enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D, float alpha = 1,
            int zeroval = 0, GFXBOOL force = GFXFALSE, int max_dimension_size = 65536,
            GFXBOOL detail_texture = GFXFALSE, GFXBOOL nocache = false,
            enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE, Texture *main = 0);

    /// Creates a texture with only color data as a single bitmap
    Texture(const char *FileName, int stage = 0, enum FILTER mipmap = MIPMAP, enum TEXTURE_TARGET target = TEXTURE2D,
            enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D, GFXBOOL force = GFXFALSE,
            int max_dimension_size = 65536, GFXBOOL detail_texture = GFXFALSE, GFXBOOL nocache = false,
            enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE, Texture *main = 0);
    Texture(VSFileSystem::VSFile *f, int stage = 0, enum FILTER mipmap = MIPMAP, enum TEXTURE_TARGET target = TEXTURE2D,
            enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D, GFXBOOL force = GFXFALSE,
            int max_dimension_size = 65536, GFXBOOL detail_texture = GFXFALSE, GFXBOOL nocache = false,
            enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE, Texture *main = 0);
    void Load(const char *, const char *, int stage = 0, enum FILTER mipmap = MIPMAP,
              enum TEXTURE_TARGET target = TEXTURE2D, enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D,
              float alpha = 1, int zeroval = 0, GFXBOOL force = GFXFALSE, int max_dimension_size = 65536,
              GFXBOOL detail_texture = GFXFALSE, GFXBOOL nocache = false,
              enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE, Texture *main = 0);
    void Load(const char *FileName, int stage = 0, enum FILTER mipmap = MIPMAP, enum TEXTURE_TARGET target = TEXTURE2D,
              enum TEXTURE_IMAGE_TARGET imagetarget = TEXTURE_2D, GFXBOOL force = GFXFALSE,
              int max_dimension_size = 65536, GFXBOOL detail_texture = GFXFALSE, GFXBOOL nocache = false,
              enum ADDRESSMODE address_mode = DEFAULT_ADDRESS_MODE, Texture *main = 0);
    ...
    {
        return 1;
    } // Number of layers in the texture. If more than 1, makeActive() sets up multitexturing units, and other use of
      // multitexture units, other than [stage..stage+numLayers), should be avoided. If not enough multitexturing units
      // are available, special fall-back mechanisms should be implmemented by MakeActive() so that it still renders as
      // correctly as possible. In that case, canMultiPass(), numPasses() and MakeActive(stage,pass) could be used to
      // process in multipass mode (one stage at a time, or a stage group at a time, depending on fallback
    ...
    {
        return true;
    } // True, if full functionality can be achieved after multipass rendering. If false, fallback rendering must be
      // possible by multipass (or singlepass) rendering. True, also, if texturing units are sufficient for singlepass
      // rendering.
    ...
    virtual bool SetupPass(int pass, const enum BLENDFUNC src, const enum BLENDFUNC dst)
    {
      ...
      // SetupPass() sets up blending. If it returns false, then blending is not compatible with the requested blend mode
      // emulation. One may assume that if numPasses()==1, no SetupPass() is needed. pass==-1 means restore setup. You
      // should call it after multipass rendering.
