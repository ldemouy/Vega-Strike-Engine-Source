    ...
    else if (target->isUnit() == UNITPTR)
    {
        if (fg)
        {
            ...
            if (fg->name != "Base" && fg->name != "Asteroid" && fg->name != "Nebula")
            {
                ...
                if (printfgname)
                    fgname += fg->name + (printshiptype ? ((confignums && ("" != fgnstring)) ? " =" : " : ") : "");
...
void VDU::DrawTarget(GameCockpit *cp, Unit *parent, Unit *target)
{
    ...
    DrawHUDSprite(this, ((target->isUnit() != PLANETPTR || target->getHudImage() != NULL) ? target->getHudImage() : (target->GetDestinations().size() != 0 ? getJumpImage() : (((Planet *)target)->hasLights() ? getSunImage() : (target->getFullname().find("invisible") != string::npos ? getNavImage() : getPlanetImage())))), .6, x, y, w, h,
                  au, ar, al, ad,
...
void VDU::DrawComm()
{
    ...
    else
    {
        ...
        if (automatte)
            tp->bgcol = GFXColor(0, 0, 0, background_alpha);
        tp->Draw(message_prefix + MangleString(_Universe->AccessCockpit()->communication_choices.c_str(),
                                               _Universe->AccessCamera()->GetNebula() != NULL ? .4 : 0),
    ...
    for (unsigned int i = 0; i < numCargo; i++)
        if ((target->GetCargo(i).GetCategory().find("upgrades/") != 0) && (target->GetCargo(i).quantity > 0))
        {
            ...
            if (((target == parent) || (maxCargo + i >= numCargo) || lastCat.compare(cc)) && (maxCargo > 0))
            {
                ...
                if (!simple_manifest)
                    retval += string(" ") + tostring(((target == parent) ? cq : 1) * (int)cp) + string("Cr.");
                retval += "\n";
            }
        }
    if (target == parent && !simple_manifest)
