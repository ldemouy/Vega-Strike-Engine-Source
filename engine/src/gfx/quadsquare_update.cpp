...
void quadsquare::Update(const quadcornerdata &cd, const Vector &ViewerLocation, float Detail, unsigned short numstages,
                        unsigned short whichstage, updateparity *par)
{
                           ...
                           unsigned int whichChildren)
{
    ...
    if (EnabledFlags == 0 && cd.Parent != nullptr &&
...
float quadsquare::RecomputeErrorAndLighting(const quadcornerdata &cd)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        else
        {
            ...
            Error[i + 2] = (unsigned short)(fabs((double)((Vertex[0].Y + cd.Verts[i].Y) -
    ...
    V->SetNormal(MakeLightness((Vertex[1].Y - Vertex[3].Y) * OneOverSize, (Vertex[4].Y - Vertex[2].Y) * OneOverSize,
                               V->GetConstVertex()));
    float v;
    quadsquare *s = GetFarNeighbor(0, cd);
    if (s)
    ...
    V->SetNormal(MakeLightness((v - Vertex[0].Y) * OneOverSize, (cd.Verts[3].Y - cd.Verts[0].Y) * OneOverSize,
                               V->GetConstVertex()));
    s = GetFarNeighbor(1, cd);
    if (s)
        v = s->Vertex[0].Y;
    ...
    V->SetNormal(MakeLightness((cd.Verts[0].Y - cd.Verts[1].Y) * OneOverSize, (Vertex[0].Y - v) * OneOverSize,
                               V->GetConstVertex()));
    s = GetFarNeighbor(2, cd);
    if (s)
        v = s->Vertex[0].Y;
    ...
    V->SetNormal(MakeLightness((Vertex[0].Y - v) * OneOverSize, (cd.Verts[2].Y - cd.Verts[1].Y) * OneOverSize,
                               V->GetConstVertex()));
    ...
    V->SetNormal(MakeLightness((cd.Verts[3].Y - cd.Verts[2].Y) * OneOverSize, (v - Vertex[0].Y) * OneOverSize,
                               V->GetConstVertex()));
    vertices->EndMutate();
