                           ...
                           unsigned int whichChildren)
{
    ...
    if ((EnabledFlags & 1) == 0 && VertexTest(cd.xorg + (half << 1), Vertex[1].Y, cd.zorg + half, Error[0], ViewerLocation) == true)
    {
        EnableEdgeVertex(
            0,
            false, cd); //East vert.
    }
    if ((EnabledFlags & 8) == 0 && VertexTest(cd.xorg + half, Vertex[4].Y, cd.zorg + (half << 1), Error[1], ViewerLocation) == true)
    {
        EnableEdgeVertex(
            3,
            false,
...
float quadsquare::RecomputeErrorAndLighting(const quadcornerdata &cd)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        else
        {
            ...
            Error[i + 2] =
                (unsigned short)(fabs((double)((Vertex[0].Y + cd.Verts[i].Y) - (Vertex[i + 1].Y + Vertex[((i + 1) & 3) + 1].Y))) * 0.25);
        }
