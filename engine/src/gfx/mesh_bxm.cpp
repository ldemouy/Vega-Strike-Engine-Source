...
string inverseblend[16] = {"ZERO",         "ZERO",          "ONE",          "SRCCOLOR",
                           "INVSRCCOLOR",  "SRCALPHA",      "INVSRCALPHA",  "DESTALPHA",
                           "INVDESTALPHA", "DESTCOLOR",     "INVDESTCOLOR", "SRCALPHASAT",
                           "CONSTALPHA",   "INVCONSTALPHA", "CONSTCOLOR",   "INVCONSTCOLOR"};
...
#define BEGIN_GL_LINES(expectitems)                                                                                    \
    ...
    {                                                                                                                  \
        ...
        xml.num_vertices = 2;                                                                                          \
        xml.active_list->reserve(2 * expectitems);                                                                     \
        xml.active_ind->reserve(2 * expectitems);                                                                      \
    } while (0)
...
#define BEGIN_GL_TRIANGLES(expectitems)                                                                                \
    ...
    {                                                                                                                  \
        ...
        xml.num_vertices = 3;                                                                                          \
        xml.active_list->reserve(3 * expectitems);                                                                     \
        xml.active_ind->reserve(3 * expectitems);                                                                      \
    } while (0)

#define BEGIN_GL_TRIANGLE(expectitems)                                                                                 \
    do                                                                                                                 \
    {                                                                                                                  \
        xml.trishade.push_back(flatshade);                                                                             \
    } while (0)
...
#define BEGIN_GL_QUADS(expectitems)                                                                                    \
    ...
    {                                                                                                                  \
        ...
        xml.num_vertices = 4;                                                                                          \
        xml.active_list->reserve(4 * expectitems);                                                                     \
        xml.active_ind->reserve(4 * expectitems);                                                                      \
    } while (0)

#define BEGIN_GL_QUAD(expectitems)                                                                                     \
    do                                                                                                                 \
    {                                                                                                                  \
        xml.quadshade.push_back(flatshade);                                                                            \
    } while (0)

#define BEGIN_GL_TRIANGLE_FAN(expectitems)                                                                             \
    do                                                                                                                 \
    {                                                                                                                  \
        xml.trifans.push_back(std::vector<GFXVertex>());                                                               \
        xml.active_list = &xml.trifans.back();                                                                         \
        xml.tfancnt = xml.trifanind.size();                                                                            \
        xml.active_ind = &xml.trifanind;                                                                               \
        xml.active_list->reserve(expectitems);                                                                         \
...
#define BEGIN_GL_QUAD_STRIP(expectitems)                                                                               \
    ...
    {                                                                                                                  \
        ...
        xml.active_list = &xml.quadstrips.back();                                                                      \
...
#define BEGIN_GL_LINE_STRIP(expectitems)                                                                               \
    ...
    {                                                                                                                  \
        ...
        xml.active_list = &xml.linestrips.back();                                                                      \
...
#define END_GL_TRIANGLE_FAN                                                                                            \
    do                                                                                                                 \
    {                                                                                                                  \
        xml.nrmltrifan.reserve(xml.trifanind.size() * 3);                                                              \
...
#define END_GL_QUAD_STRIP                                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        xml.nrmlquadstrip.reserve(xml.quadstripind.size() * (4 / 2));                                                  \
...
#define READSTRING(inmemfile, word32index, stringlen, stringvar)                                                       \
    ...
    { /* By Klauss - Much more efficient than the preceding code, and yet still portable */                            \
        ...
        uchar8bit oc = inmemstring[stringlen];                                                                         \
        inmemstring[stringlen] = 0;                                                                                    \
...
#define BEGINSTATE(stat, expectitems)                                                                                  \
    do                                                                                                                 \
    {                                                                                                                  \
        BEGIN_##stat(expectitems);                                                                                     \
        DLISTBEGINSTATE(stat);                                                                                         \
    } while (0)
...
vector<Mesh *> Mesh::LoadMeshes(VSFileSystem::VSFile &Inputfile, const Vector &scalex, int faction,
                                class Flightgroup *fg, std::string hash_name,
                                const std::vector<std::string> &overrideTextures)
{
    ...
    uint32bit NUMFIELDSPERPOLYGONSTRUCTURE =
        VSSwapHostIntToLittle(inmemfile[word32index + 2].i32val); // Number of fields per polygon structure: integer (1)
    uint32bit NUMFIELDSPERREFERENCEDVERTEX =
        VSSwapHostIntToLittle(inmemfile[word32index + 3].i32val); // Number of fields per referenced vertex: integer (3)
    uint32bit NUMFIELDSPERREFERENCEDANIMATION = VSSwapHostIntToLittle(
        inmemfile[word32index + 4].i32val); // Number of fields per referenced animation: integer (1)
    uint32bit numrecords = VSSwapHostIntToLittle(inmemfile[word32index + 5].i32val); // Number of records: integer
    uint32bit NUMFIELDSPERANIMATIONDEF =
        VSSwapHostIntToLittle(inmemfile[word32index + 6].i32val); // Number of fields per animationdef: integer (1)
    ...
    for (uint32bit recordindex = 0; recordindex < numrecords; recordindex++)
    {
        ...
        for (uint32bit meshindex = 0; meshindex < nummeshes; meshindex++)
        {
            ...
            uint32bit LengthOfArbitraryLengthAttributes = VSSwapHostIntToLittle(
                inmemfile[word32index].i32val); // Length of Arbitrary length attributes section in bytes
            word32index += 1;
            bxmfprintf(Outputfile,
                       "<Mesh scale=\"%f\" reverse=\"%d\" forcetexture=\"%d\" sharevert=\"%d\" polygonoffset=\"%f\" "
                       "blendmode=\"%s %s\" alphatest=\"%f\" ",
                       scale, reverse, forcetexture, sharevert, polygonoffset, inverseblend[bsrc % 16].c_str(),
                       inverseblend[bdst % 16].c_str(), alphatest);
            ...
            if (detailtexturename.size() != 0)
            {
                ...
                mesh->detailTexture =
                    mesh->TempGetTexture(&xml, detailtexturename, FactionUtil::GetFaction(xml.faction),
                                         GFXTRUE); // LoadTexture(detailtexturename);
            }
            else
            {
            ...
            for (uint32bit tex = 0; tex < numtextures; tex++)
            {
                ...
                uint32bit texindex = VSSwapHostIntToLittle(inmemfile[word32index + 1].i32val);   // texture index
                uint32bit texnamelen = VSSwapHostIntToLittle(inmemfile[word32index + 2].i32val); // texture name length
                word32index += 3;
                string texname = "";
            ...
            static bool forcelight =
                XMLSupport::parse_bool(vs_config->getVariable("graphics", "ForceLighting", "true"));
            ...
            for (uint32bit detplane = 0; (unsigned int)detplane < Detailplanes.size(); detplane++)
            {
                ...
                mesh->detailPlanes.push_back(
                    Vector(Detailplanes[detplane].x, Detailplanes[detplane].y, Detailplanes[detplane].z));
            }
            // Logos
            ...
            for (uint32bit logo = 0; logo < numlogos; logo++)
            {
                ...
                uint32bit numrefs =
                    VSSwapHostIntToLittle(inmemfile[word32index + 4].i32val); // number of reference points
                ...
                {
                    uint32bit refnum = VSSwapHostIntToLittle(inmemfile[word32index].i32val);        // Logo ref
            ...
            {
                uint32bit animnamelen = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // length of name
                word32index += 1;
                string animname;
            ...
            uint32bit numvertices = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
            word32index += 1;
            ...
            // End Quads
            // Linestrips
            uint32bit numlinestrips = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
            word32index += 1;
            for (uint32bit lstrip = 0; lstrip < numlinestrips; lstrip++)
            {
                uint32bit numstripelements = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
                uint32bit flatshade = VSSwapHostIntToLittle(inmemfile[word32index + 1].i32val);    // flatshade
            ...
            // End Linestrips
            // Tristrips
            uint32bit numtristrips = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
            word32index += 1;
            ...
            // End Tristrips
            // Trifans
            uint32bit numtrifans = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
            word32index += 1;
            ...
            // End Trifans
            // Quadstrips
            uint32bit numquadstrips = VSSwapHostIntToLittle(inmemfile[word32index].i32val); // number of vertices
            word32index += 1;
