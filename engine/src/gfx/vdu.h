...
class VDU : public VSSprite
{
  private:
    ...
    /// Draws the sprite representing the current starship. Returns proper location and size for future use (with weaps
    /// and damage display)
  ...
  public:
    ...
    VDU(const char *file, TextPlane *textp, unsigned short modes, short rows, short cols, float *MaxArmor,
        float *maxhull);
