...
class MeshFX : public GFXLight
{
  public:
    ...
    MeshFX(const float TTL, const float delta, const bool enabled, const GFXColor &vect,
           const GFXColor &diffuse = GFXColor(0, 0, 0, 1), const GFXColor &specular = GFXColor(0, 0, 0, 1),
           const GFXColor &ambient = GFXColor(0, 0, 0, 1), const GFXColor &attenuate = GFXColor(1, 0, 0));
...
class Mesh
{
  private:
    ...
    void LoadXML(const char *filename, const Vector &scale, int faction, class Flightgroup *fg, bool orig,
                 const vector<string> &overrideTexture);
    void LoadXML(VSFileSystem::VSFile &f, const Vector &scale, int faction, class Flightgroup *fg, bool orig,
                 const vector<string> &overrideTexture);
  ...
  protected:
    // only may be called from subclass. orig request may be denied if unit was in past usage. (not likely in the case
    // where a unit must be constructed in orig)
  ...
  public:
    ...
    void Draw(float lod, const Matrix &m = identity_matrix, float toofar = 1, int cloak = -1, float nebdist = 0,
              unsigned char damage = 0, bool renormalize_normals = false,
              const MeshFX *mfx = nullptr); // short fix
                                            /// Draws lod pixels wide, mesh at Transformation NOW. If centered, then
                                            /// will center on camera and disable cull
    void DrawNow(float lod, bool centered, const Matrix &m = identity_matrix, int cloak = -1,
    ...
    /// Will draw all undrawn meshes in total If pushSpclFX, the last series of meshes will be drawn with other lighting
    /// off
    ...
    void AddDamageFX(const Vector &LocalPos, const Vector &LocalNorm, const float percentage,
