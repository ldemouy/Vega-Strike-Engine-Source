...
static int32_t rnd(int32_t in1, int32_t in2)
{
    return (int32_t)(in1 + (in2 - in1) * (float(rand())) / (((float)RAND_MAX) + 1));
}

static int32_t iav(int32_t in1, int32_t in2)
{
    return rnd(in1, in2 + 1);
}

static int32_t isqav(int32_t in1, int32_t in2)
{
    return (int32_t)(in1 + (in2 + 1 - in1) * sqav01());
}
...
void MakeStarSystem(string file, Galaxy *galaxy, string origin, int32_t forcerandom)
{
    SystemInfo Ave;
    SystemInfo si;
...
void Universe::getJumpPath(const std::string &from, const std::string &to, vector<std::string> &path) const
{
    ...
    path.clear();
    map<string, uint32_t>::const_iterator velem = visited.find(to);
