...
/* Debug level is a character, 1, 2, or 3.   1 is most important warnings only. 2 is warnings that are important but not
 * hazardous to gameplay 3 is for developer type messages, more akin to informational output but not relevant to
 * anything an end user would care about. example usage.   vegastrike --debug=3       leaving --debug empty as an
 * argument is the same as --debug=1 all output is sent to stderr
 */
void vs_dprintf(char level, const char *format, ...);

template <class A> int vs_fscanf(FILE *fp, const char *format, A *a)
{
    return fscanf(fp, format, a);
}
template <class A, class B> int vs_fscanf(FILE *fp, const char *format, A *a, B *b)
{
    return fscanf(fp, format, a, b);
}
template <class A, class B, class C> int vs_fscanf(FILE *fp, const char *format, A *a, B *b, C *c)
{
    return fscanf(fp, format, a, b, c);
}
template <class A, class B, class C, class D> int vs_fscanf(FILE *fp, const char *format, A *a, B *b, C *c, D *d)
{
 ...
 *  // We need 1 arg more than vs_fscanf because we pass an extra arg to get the number of bytes read by sscanf in the
 * last arg template< class A, class B, class C, class D, class E, class F, class G> int	vs_sscanf( const char * buf,
 * const char * format, A * a, B * b, C * c, D * d, E * e, F * f, G * g) { return sscanf( buf, newformat, a, b, c, d, e,
 * f); }
 */
 ...
 *                                                                                                               // Note
 * : may only change ONE level of directory or it breaks
...
VSError GetError(const char *str = nullptr);
...
class VSFile
{
  ...
  public:
    ...
    template <class A> int Fscanf(const char *format, A *a)
    {
        int ret = -1;
        ...
        else
        {
            if (q_volume_format == vfmtPK3)
            {
                ...
                delete[] newformat;
                std::cerr << "FSCANF : sscanf read " << readbytes << " bytes - OFFSET=" << offset
                          << " VALUES : a=" << (*a) << std::endl;
    ...
    template <class A, class B> int Fscanf(const char *format, A *a, B *b)
    {
        int ret = -1;
        if (!UseVolumes[this->alt_type])
        {
    ...
    template <class A, class B, class C> int Fscanf(const char *format, A *a, B *b, C *c)
    {
        int ret = -1;
        if (!UseVolumes[this->alt_type])
        {
    ...
    template <class A, class B, class C, class D> int Fscanf(const char *format, A *a, B *b, C *c, D *d)
    {
        int ret = -1;
        if (!UseVolumes[this->alt_type])
        {
    ...
    template <class A, class B, class C, class D, class E> int Fscanf(const char *format, A *a, B *b, C *c, D *d, E *e)
    {
        int ret = -1;
        if (!UseVolumes[this->alt_type])
        {
