...
namespace StarSystemGent
{
    ...
    struct PlanetInfo
    {
        ...
        uint32_t numstarbases; //Number of starbases allocated to orbit around this planet. Usually 1 or 0 but quite possibly more.
    ...
    void MakeBigUnit(int callingentitytype, string name = string(), float orbitalradius = 0)
    {
        ...
        for (size_t i = 0; i < fullname.size(); i++)
        {
            ...
            else if (1 == sscanf(fullname[i].c_str(), "planet%f",
                                 &size) ||
    ...
    void MakePlanet(float radius, int32_t entitytype, string texturename, string unitname, string technique, int32_t texturenum, int32_t numberofjumps, int32_t numberofstarbases)
    {
        if (entitytype == JUMP)
        {
            MakeJump(radius);
