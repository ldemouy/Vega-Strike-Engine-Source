...
Unit *launchJumppoint(string name_string, string faction_string, string type_string, string unittype_string,
                      string ai_string, int nr_of_ships, int nr_of_waves, QVector pos, string squadlogo,
                      string destinations)
{
    int clstype = UNITPTR;
    ...
    cf.fg = Flightgroup::newFlightgroup(name_string, type_string, faction_string, ai_string, nr_of_ships, nr_of_waves,
                                        squadlogo, "", mission);
...
Unit *launch(string name_string, string type_string, string faction_string, string unittype, string ai_string,
             int nr_of_ships, int nr_of_waves, QVector pos, string sqadlogo)
{
    return launchJumppoint(name_string, faction_string, type_string, unittype, ai_string, nr_of_ships, nr_of_waves, pos,
                           sqadlogo, "");
}
...
{
    cachedUnits.push_back(
        UnitFactory::createUnit(type_string.c_str(), true, FactionUtil::GetFactionIndex(faction_string)));
}
Unit *getPlayer()
{
...
void receivedCustom(int cp, bool trusted, string cmd, string args, string id)
{
    ...
    securepythonstr(id);
    string pythonCode = game_options.custompython + "(" + (trusted ? "True" : "False") + ", r\'" + cmd + "\', r\'" +
                        args + "\', r\'" + id + "\')\n";
...
static std::string simplePrettySystem(std::string system)
{
    ...
    return std::string("Sec:") + system.substr(0, where) + " Sys:" +
           (where == string::npos
                ? std::string("")
...
string getSaveInfo(const std::string &filename, bool formatForTextbox)
{
    ...
    if (!campaign_score_vars_init)
    {
        ...
        {
            campaign_score_vars.insert(
                game_options.campaigns.substr(where, ((when == string::npos) ? when : when - where)));
    ...
    savegame.ParseSaveGame(filename, system, "", pos, updatepos, creds, Ships, _Universe->CurrentCockpit(), "", true,
                           false, game_options.quick_savegame_summaries, true, true, campaign_score_vars);
    ...
    text += "Credits: " + XMLSupport::tostring((unsigned int)creds) + "." +
            XMLSupport::tostring(((unsigned int)(creds * 100)) % 100) + lf;
    ...
    if (Ships.size())
    {
        ...
        if (Ships.size() > 2)
        {
            ...
            for (unsigned int i = 2; i < Ships.size(); i += 2)
                text += " " + simplePrettyShip(Ships[i - 1]) + lf + "  Located At:" + lf + "  " +
