...
#define DEFAULT_FACTION_SAVENAME "FactionTookOver_"
    ...
    void receivedCustom(int cp, bool trusted, string cmd, string args, string id)
    {
        ...
        securepythonstr(id);
        string pythonCode = game_options.custompython + "(" + (trusted ? "True" : "False") + ", r\'" + cmd + "\', r\'" + args + "\', r\'" + id + "\')\n";
    ...
    static std::string simplePrettySystem(std::string system)
    {
        ...
        return std::string("Sec:") + system.substr(0, where) + " Sys:" + (where == string::npos ? std::string("") : system.substr(where + 1, (basewhere != string::npos) ? basewhere - where - 1 : string::npos)) + (basewhere == string::npos ? std::string("") : std::string(" ") + system.substr(basewhere + 1));
    }
