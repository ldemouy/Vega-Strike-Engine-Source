...
float AUDDistanceSquared(const int32_t &sound)
{
#ifdef HAVE_AL
...
static float EstimateGain(const Vector &pos, const float &gain)
{
    ...
    float distance = (AUDListenerLocation() - pos.Cast()).Magnitude() - listener_size - game_options.audio_ref_distance;
...
char AUDQueryAudability(const int32_t &sound, const Vector &pos, const Vector &vel, const float &gain)
{
#ifdef HAVE_AL
...
void AUDSoundGain(int32_t sound, float gain, bool music)
{
#ifdef HAVE_AL
