...
/// this function sets the "current" system to be "name"  where name may be something like "Sol/Sol" or
/// "Crucible/Cephid_17"  this function may take some time if the system has not been loaded before

void pushSystem(std::string name);
bool systemInMemory(std::string name);

/// this function restores the active system.... there must be an equal number of pushSystems ans popSystems or else
/// Vega Strike may behave unpredictably
void popSystem();
...
/// this function gets an iterator into the units in the current system... do NOT keep an iterator across a frame--it
/// may get deleted!
...
Unit *launchJumppoint(std::string name_string, std::string faction_string, std::string type_string,
                      std::string unittype_string, std::string ai_string, int nr_of_ships, int nr_of_waves, QVector pos,
                      std::string squadlogo, std::string destinations);
std::string vsConfig(std::string category, std::string option, std::string def);

/// this function launches a normal fighter  the name is the flightgroup name, the type is the ship type, the faction is
/// who it belongs to, the unittype is usually "unit" unless you want to make asteroids or nebulae or jump points or
/// planets.  the aistring is either a python filename or "default"  the nr of ships is the number of ships to be
/// launched with this group, the number of waves is num reinforcements... the position is a tuple (x,y,z) where they
/// appear and the squadlogo is a squadron image...you can leave this the empty string '' for the default squadron logo.
Unit *launch(std::string name_string, std::string type_string, std::string faction_string, std::string unittype,
             std::string ai_string, int nr_of_ships, int nr_of_waves, QVector pos, std::string sqadlogo);

/// this gets a random cargo type (useful for cargo missions) from either any category if category is '' or else from a
/// specific category  'Contraband'  comes to mind!
...
/// this gets a specific property of this system as found in universe/milky_way.xml and returns a default value if not
/// found
...
/// this gets the difficutly of the game... ranges between 0 and 1... many missions depend on it never going past .99
/// unless it's always at one.
float GetDifficulty();
...
/// tells the respective flightgroups in this system to stop killing each other urgently...they may still attack--just
/// not warping and stuff
...
/// this sends an IO message... I'm not sure if delay currently works, but from, to and message do :-) ... if you want
/// to send to the bar do "bar" as the to string... if you want to make news for the news room specify "news"
...
/// this sets whether or not a player may autopilot.  Normally they are both 0 and the autopiloting is allowed based on
/// if enemies are near... if you pass in 1 then autopilot will be allowed no matter who is near... if you set -1 then
/// autopilot is never allowed.  global affects all players... player just affects the player who accepted the mission.
