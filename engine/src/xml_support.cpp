...
namespace XMLSupport
{
    ...
    string::size_type parse_option_find(const string &str, const string &opt, const string &sep, const string &vsep)
    {
        ...
        {
            ini = false;
            found = (((pos == 0) || (sep.find(str[pos - 1]) != string::npos)) && ((pos + optlen >= strlen) || (allsep.find(str[pos + optlen]) != string::npos)));
            if (!found)
                pos = str.find_first_of(sep, pos + optlen); //quick advancement
        }
