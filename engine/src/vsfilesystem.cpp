...
#endif
{
    ...
    if ((string(entry->d_name).find(".pk3")) != std::string::npos &&
        (string(entry->d_name).find("data")) == std::string::npos)
        return 1;
    return 0;
}
...
void InitHomeDirectory()
{
    ...
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_PERSONAL, nullptr, 0, szPath)))
...
void LoadConfig(string subdir)
{
    ...
    if (!found)
    {
        ...
        {
            cerr << "CONFIGFILE - Found a config file in home directory, using : " << (homedir + "/" + config_file)
                 << endl;
        ...
        else
        {
            ...
            {
                cerr << "CONFIGFILE - No home config file found, using datadir config file : "
                     << (datadir + "/" + config_file) << endl;
...
void InitPaths(string conf, string subdir)
{
  ...
  Units are in sharedunits/unitname/, sharedunits/subunits/unitname/ or sharedunits/weapons/unitname/ or in
  sharedunits/faction/unitname/ Meshes are in sharedmeshes/ or in current unit that is being loaded Textures are in
  sharedtextures/ or in current unit dir that is being loaded or in the current animation dir that is being loaded or in
  the current sprite dir that is being loeded Sounds are in sharedsounds/ Universes are in universe/ Systems are in
  "sectors"/ config variable Cockpits are in cockpits/ (also a config var) Animations are in animations/ VSSprite are in
  sprites/ or in ./ (when full subpath is provided) or in the current cockpit dir that is being loaded First allocate an
  empty directory list for each file type
*/
    for (i = 0; i < UnknownFile; i++)
    {
        vector<string> vec;
    ...
    {
        // Every kind of file will use the big volume except Unknown files and python files that needs to remain
        // standard files
        for (i = 0; i < UnknownFile; i++)
            UseVolumes[i] = 2;
    ...
    if (q_volume_format == vfmtPK3)
    {
        if (!pk3_extracted_file)
        {
            ...
            if (this->file_index != -1)
                ...
                pk3_extracted_file =
                    (char *)pk3_file->ExtractFile((this->subdirectoryname + "/" + this->filename).c_str(), &pk3size);
            this->size = pk3size;
            cerr << "EXTRACTING " << (this->subdirectoryname + "/" + this->filename)
                 << " WITH INDEX=" << this->file_index << " SIZE=" << pk3size << endl;
        }
    }
}
    ...
    if (type < ZoneBuffer || type == UnknownFile)
    {
        ...
        if (!UseVolumes[type])
        {
            if (type == UnknownFile)
            {
                ...
                if (current_path.back() != "")
                {
                    string filestr1 = current_directory.back() + "/" + current_subdirectory.back() + "/" + string(file);
                ...
                else
                {
                    ...
                    {
                        cerr << "!!! SERIOUS ERROR : failed to open Unknown file " << filestr
...
string VSFile::ReadFull()
{
    if (this->Size() < 0)
    {
        cerr << "Attempt to call ReadFull on a bad file " << this->filename << " " << this->Size() << " "
