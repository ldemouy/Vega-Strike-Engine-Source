...
void /*GFXDRVAPI*/ GFXClear(const GFXBOOL colorbuffer, const GFXBOOL depthbuffer = GFXTRUE,
...
/// Creates a new material given a material struct.  Will search through already made materials for matching materials
...
/// Sets the Projection matrix to have fov and aspect as follows (fov is field of view in radians, aspect is
/// width/height znear and zfar are clip planes
...
/// Given matrices, calculates the matrix and inverse matrix of a projection matrix to go from screen to 3-space
/// coordinates
void /*GFXDRVAPI*/ GFXFrustum(float *mat, float *inv, float left, float right, float bottom, float top, float nearval,
                              float farval);
 ...
 */
GFXBOOL /*GFXDRVAPI*/ GFXCreateTexture(int width, int height, TEXTUREFORMAT externaltextureformat, int *handle,
                                       char *palette = 0, int texturestage = 0, enum FILTER mipmap = MIPMAP,
 ...
 */
GFXBOOL /*GFXDRVAPI*/ GFXTransferTexture(
    unsigned char *buffer, int handle, int inWidth, int inHeight, enum TEXTUREFORMAT internalformat,
    ...
    GFXBOOL detailtexture = GFXFALSE, unsigned int pageIndex = 0);

GFXBOOL /*GFXDRVAPI*/ GFXTransferSubTexture(unsigned char *buffer, int handle, int x, int y, unsigned int width,
                                            unsigned int height, enum TEXTURE_IMAGE_TARGET image2D = TEXTURE_2D);
...
void /*GFXDRVAPI*/ GFXDraw(POLYTYPE type, const float data[], int vnum, int vsize = 3, int csize = 0, int tsize0 = 0,
                           int tsize1 = 0);

void /*GFXDRVAPI*/ GFXDrawElements(POLYTYPE type, const float data[], int vnum, const unsigned char indices[],
                                   int nelem, int vsize = 3, int csize = 0, int tsize0 = 0, int tsize1 = 0);
...
void GFXDrawElements(POLYTYPE type, const VertexBuilder<float, VSIZE, 0, CSIZE, TSIZE0, TSIZE1> &buffer,
                     const ITYPE *indices, int nelements)
{
                     ...
                     const std::vector<ITYPE> &indices)
{
    if (buffer.size() > 0 && indices.size() > 0)
        GFXDrawElements(type, buffer.buffer_pointer(), buffer.size(), &indices[0], indices.size(), VSIZE, CSIZE, TSIZE0,
                        TSIZE1);
}
