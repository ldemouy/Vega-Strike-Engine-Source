...
static bool nameIsAsteroid(std::string name)
{
    if (name.length() < 8)
        return false;
    return (name[0] == 'A' || name[0] == 'a') && name[1] == 's' && name[2] == 't' && name[3] == 'e' && name[4] == 'r' && name[5] == 'o' && name[6] == 'i' && name[7] == 'd';
}
...
namespace UnitUtil
{
            ...
            if (player)
            {
                ...
                if (tmpdist < cpdist)
                {
                    ...
                    if (relpos.Dot(relvel) >= 0)
                    {
                        ...
                        tooclose =
                            2 * (un->radial_size + player->radial_size) + relvel.Magnitude() * lowest_priority_time;
                    }
                }
            }
#ifndef NO_GFX
            ...
            if (cam)
            {
                ...
                if (dist < cpdist)
                {
                    ...
                    if (relpos.Dot(relvel) >= 0)
                        //No need to be wary if they're getting away
                        tooclose =
                            2 * (un->radial_size + (parent ? parent->radial_size : 0)) + relvel.Magnitude() * lowest_priority_time;
                }
            }
#endif
    ...
    bool isDockableUnit(const Unit *my_unit)
    {
        ...
        return (
                   (
                       my_unit->isPlanet() && !isSun(my_unit) && isSignificant(my_unit) && !my_unit->isJumppoint()) ||
    ...
    bool isCloseEnoughToDock(const Unit *my_unit, const Unit *un)
    {
        ...
        float dis =
            (un->isUnit() == PLANETPTR || superdock) ? UnitUtil::getSignificantDistance(my_unit, un) : UnitUtil::getDistance(my_unit, un);
        if (dis < un->rSize())
            return true;
        return false;
    }
