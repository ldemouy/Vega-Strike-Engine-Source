...
static bool nameIsAsteroid(std::string name)
{
    if (name.length() < 8)
        return false;
    return (name[0] == 'A' || name[0] == 'a') && name[1] == 's' && name[2] == 't' && name[3] == 'e' && name[4] == 'r' &&
           name[5] == 'o' && name[6] == 'i' && name[7] == 'd';
}
...
template <typename T> static inline T mymin(T a, T b)
{
    return (a < b) ? a : b;
}

template <typename T> static inline T mymax(T a, T b)
{
    return (a > b) ? a : b;
}
...
int getPhysicsPriority(Unit *un)
{
    ...
    static const int PLAYER_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "player", "1"));
    static const int MISSILE_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "missile", "1"));
    static const int DOCKABLE_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "dockable", "1"));
    ...
    static const int SYSTEM_INSTALLATION_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "system_installation", "3"));
    bool system_installation = un->owner == getTopLevelOwner();
    bool force_system_installation_priority = false;
    if (system_installation &&
        un->Velocity.MagnitudeSquared() > fixed_system_orbit_priorities * fixed_system_orbit_priorities)
    ...
    for (unsigned int i = 0; i < np; ++i)
    {
        ...
        if (player)
        {
            ...
            if (tmpdist < cpdist)
            {
                ...
                if (relpos.Dot(relvel) >= 0)
                {
                    ...
                    tooclose = 2 * (un->radial_size + player->radial_size) + relvel.Magnitude() * lowest_priority_time;
                }
            }
        }
#ifndef NO_GFX
        ...
        if (cam)
        {
            ...
            if (dist < cpdist)
            {
                ...
                if (relpos.Dot(relvel) >= 0)
                    // No need to be wary if they're getting away
                    tooclose = 2 * (un->radial_size + (parent ? parent->radial_size : 0)) +
                               relvel.Magnitude() * lowest_priority_time;
            }
        }
#endif
        if (player == un || (0 && _Universe->AccessCockpit(i)->GetSaveParent() == un))
            return PLAYER_PRIORITY;
    }
    ...
    static const int ASTEROID_PARENT_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "asteroid_parent", "1"));
    static const int ASTEROID_HIGH_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "asteroid_high", "2"));
    static const int ASTEROID_LOW_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "asteroid.low", "32"));
    static const int HIGH_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "high", "2"));
    static const int MEDIUMHIGH_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "mediumhigh", "4"));
    static const int MEDIUM_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "medium", "8"));
    static const int LOW_PRIORITY = XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "low", "32"));
    static const int NOT_VISIBLE_COMBAT_HIGH =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "notvisible_combat_high", "10"));
    static const int NOT_VISIBLE_COMBAT_MEDIUM =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "notvisible_combat_medium", "20"));
    static const int NOT_VISIBLE_COMBAT_LOW =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "notvisible_combat_low", "40"));
    static const int NO_ENEMIES =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "no_enemies", "64"));
    static const int INERT_PRIORITY =
        XMLSupport::parse_int(vs_config->getVariable("physics", "priorities", "inert", "64"));
    static const float _PLAYERTHREAT_DISTANCE_FACTOR =
        XMLSupport::parse_float(vs_config->getVariable("physics", "priorities", "playerthreat_distance_factor", "2"));
    static const float _THREAT_DISTANCE_FACTOR =
        XMLSupport::parse_float(vs_config->getVariable("physics", "priorities", "threat_distance_factor", "1"));
    ...
    static const float DYNAMIC_THROTTLE_TARGETFPS =
        XMLSupport::parse_float(vs_config->getVariable("physics", "priorities", "dynamic_throttle.targetfps", "30"));
    ...
    {
        static float compwarprampuptime = XMLSupport::parse_float(vs_config->getVariable(
            "physics", "computerwarprampuptime", "10")); // for the heck of it.  NOTE, variable also in unit_generic.cpp
...
void orbit(Unit *my_unit, Unit *orbitee, float speed, QVector R, QVector S, QVector center)
{
    if (my_unit)
    {
        my_unit->PrimeOrders(new PlanetaryOrbit(my_unit, speed / (3.1415926536 * (S.Magnitude() + R.Magnitude())), 0, R,
                                                S, center, orbitee));
        if (orbitee)
...
void RecomputeUnitUpgrades(Unit *un)
{
    ...
    for (i = 0; i < un->numCargo(); ++i)
    {
        ...
        {
            if (c->GetContent().find("mult_") != 0 && c->GetContent().find("add_") != 0)
...
bool isDockableUnit(const Unit *my_unit)
{
    if (!my_unit)
        return false;
    return ((my_unit->isPlanet() && !isSun(my_unit) && isSignificant(my_unit) && !my_unit->isJumppoint()) ||
            (my_unit->isUnit() == UNITPTR) || (getFlightgroupName(my_unit) == "Base")) &&
           (my_unit->DockingPortLocations().size() > 0);
}

bool isCloseEnoughToDock(const Unit *my_unit, const Unit *un)
{
    static bool superdock =
        XMLSupport::parse_bool(vs_config->getVariable("physics", "dock_within_base_shield", "false"));
    float dis = (un->isUnit() == PLANETPTR || superdock) ? UnitUtil::getSignificantDistance(my_unit, un)
...
float PercentOperational(Unit *un, std::string name, std::string category, bool countHullAndArmorAsFull)
{
    ...
    else if (name.find("add_") != 0 && name.find("mult_") != 0)
    {
        ...
        if (upgrade->GetHull() > 1 || armor[0] || armor[1] || armor[2] || armor[3] || armor[4] || armor[5] ||
            armor[6] || armor[7])
        ...
        if (un->canUpgrade(upgrade, -1, -1, 0, true, percent, makeTemplateUpgrade(un->name, un->faction), false))
        {
            if (percent > 0 && percent < 1)
                return percent;
            else if (percent >= 1) // FIXME workaround for sensors -- see below comment, not sure why sensors report
                                   // erroneous functional percentage
                return 1.0;
            else
                return .5; // FIXME does not interact well with radar type
        }
        else if (percent > 0)
        {
            return percent;
