...
vector<vector<string>> buildscripts()
{
    ...
    if (err <= Ok)
    {
        ...
        {
            fprintf(stderr,
                    "FATAL error in hash map... column %u in ai/VegaEvents.csv does not line up with that item in "
                    "ai/VegaPriorities.csv\n",
        ...
        for (unsigned int j = 0; j < vec.size(); j++)
            ...
            {
                fprintf(stderr,
                        "FATAL error in hash map... column %d in ai/VegaEvents.csv does not line up with that item in "
                        "ai/VegaPriorities.csv\n",
                        j);
            }
        unsigned int i = 0;
...
{
    static string defaultcapshipvalues =
        vs_config->getVariable("data", "capship_roles", "ESCORTCAP CAPITAL CARRIER BASE TROOP");
