...
FireKeyboard::FireKeyboard(uint32_t whichplayer, uint32_t whichjoystick) : Order(WEAPON, 0)
{
...
bool getNearestTargetUnit(Unit *me, int iType)
{
    ...
    for (auto i = _Universe->activeStarSystem()->getUnitList().createIterator(); (un = (*i)); ++i)
    {
        ...
        if (!(me->InRange(un, true, false)) || !(me->InRange(un, true, true)))
            continue;
        if ((iType == 0) && ((un->isUnit() != UNITPTR) || !me->isEnemy(un)))
            continue;
        ...
        if ((iType == 3) && (UnitUtil::getFlightgroupName(un) != "Base"))
            continue;
        if ((iType == 4) && ((!un->isPlanet()) || (un->isJumppoint())))
            continue;
