...
/**
 * The moveto order attempts to calculate the best way to apply thrust (within the computer bound limits) to get a
 * starship to place B and stopped. It uses an integral of acceleration and velocity over time to solve for time when to
 * decelerate.  Is  inaccurate within 1 physics frame, and must use switchbacks and then once they have been met sets
 * terminating X,Y, and Z to figure out how many switchbacks it has made , missing the target and coming back over it.
 */
...
class MoveToParent
{
  ...
  public:
    ...
    MoveToParent(bool aft, uint8_t switchbacks, bool terminating = true)
        : afterburn(aft), switchbacks(switchbacks), terminatingX(0), terminatingY(0), terminatingZ(0),
          last_velocity(0, 0, 0), selfterminating(terminating)
    {
    }
...
{
    MoveToParent m;
    /// The last_velocity keeps track of the previous velocity so the script may determine if it has crossed over 0 this
    /// frame or not
  public:
    void SetAfterburn(bool tf)
    {
    ...
    /// takes in the destination target, whether afterburners should be applied, and the ammount of accuracy (how many
    /// times it shoudl miss destination and come back) should be used
    MoveTo(const QVector &target, bool aft, uint8_t switchbacks, bool terminating = true)
        : Order(MOVEMENT, SLOCATION), m(aft, switchbacks, terminating)
    {
...
class ChangeHeading : public Order
{
  ...
  public:
    /// takes in the destination target, and the ammount of accuracy (how many times it should miss destination and come
    /// back) should be used
    ChangeHeading(const QVector &final_heading, int switchback, float turning_speed = 1, bool term = false)
        : Order(FACING, SLOCATION), turningspeed(turning_speed), switchbacks(switchback), terminatingX(0),
          terminatingY(0), last_velocity(0, 0, 0), final_heading(final_heading), terminating(term)
    {
    }
    void SetDest(const QVector &);
    virtual void Execute();
