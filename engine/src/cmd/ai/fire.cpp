...
Unit *getAtmospheric(Unit *target)
{
    ...
    {
        Unit *unit;
        for (auto i = _Universe->activeStarSystem()->getUnitList().createIterator(); (unit = *i) != nullptr; ++i)
        {
...
{
    static float missileprob =
        XMLSupport::parse_float(vs_config->getVariable("AI", "Firing", "MissileProbability", ".01"));
...
struct TurretBin
{
    ...
    void AssignTargets(const TargetAndRange &finalChoice, const Matrix &pos)
    {
        ...
        for (auto turret_iter = turret.begin(); turret_iter != turret.end(); ++turret_iter)
        {
            ...
            {
                if (finalChoice.range < turret_iter->gunrange &&
                    ROLES::getPriority(turret_iter->turret->attackPreference())[finalChoice.target->unitRole()] < 31)
                {
...
float Priority(Unit *me, Unit *targ, float gunrange, float rangetotarget, float relationship, char *rolepriority)
{
    ...
    *rolepriority = ROLES::getPriority(me->attackPreference())[targ->unitRole()]; // number btw 0 and 31 higher better
    ...
    float inertial_priority = 0;
    {
        ...
        {
            static float mass_inertial_priority_scale = XMLSupport::parse_float(
                vs_config->getVariable("AI", "Targetting", "MassInertialPriorityScale", ".0000001"));
    ...
    static float threat_weight =
        XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "ThreatWeight", ".5"));
...
template <size_t numTuple> class ChooseTargetClass
{
  ...
  public:
    ...
    void init(FireAt *fireat, Unit *un, float gunrange, vector<TurretBin> *tbin,
              const StaticTuple<float, numTuple> &innermaxrange, char maxrolepriority, int maxtargets)
    {
        this->fireat = fireat;
        this->tbin = tbin;
        this->parent = un;
...
int32_t pollindex[2] = {
    1, 1}; // current count of number of units touched (doesn't need to be precise)  -- used for "fairness" heuristic
...
void FireAt::ChooseTargets(int numtargs, bool force)
{
    ...
    static int32_t minnumpollers = float_to_int(XMLSupport::parse_float(vs_config->getVariable(
        "AI", "Targetting", "MinNumberofpollersperframe",
        "5"))); // maximum number of vessels allowed to search for a target in a given physics frame
    static int32_t maxnumpollers = float_to_int(XMLSupport::parse_float(vs_config->getVariable(
        "AI", "Targetting", "MaxNumberofpollersperframe",
    ...
    // Following code exists to limit the number of craft polling for a target in a given frame - this is an expensive
    // operation, and needs to be spread out, or there will be pauses.
    ...
    pollindex[hastarg]++; // count number of craft touched - will use in the next physics frame to spread out the
                          // vessels actually chosen to be processed among all of the vessels being touched
    ...
    if (!(pollindex[hastarg] % ((prevpollindex[hastarg] / numpollers[hastarg]) +
                                1))) // spread out, in modulo fashion, the possibility of changing one's target. Use
                                     // previous physics frame count of craft to estimate current number of craft
    {
    ...
    static float unitRad = XMLSupport::parse_float(vs_config->getVariable(
        "AI", "Targetting", "search_extra_radius", "1000")); // Maximum target radius that is guaranteed to be detected
    static char maxrolepriority =
        XMLSupport::parse_int(vs_config->getVariable("AI", "Targetting", "search_max_role_priority", "16"));
    static int32_t maxtargets = XMLSupport::parse_int(vs_config->getVariable(
        "AI", "Targetting", "search_max_candidates", "64")); // Cutoff candidate count (if that many hostiles found,
                                                             // stop search - performance/quality tradeoff, 0=no cutoff)
    ...
    static int32_t min_rechoose_interval =
        XMLSupport::parse_int(vs_config->getVariable("AI", "min_rechoose_interval", "128"));
    if (curtarg)
    {
        if (gcounter++ < min_rechoose_interval || rand() / 8 < RAND_MAX / 9)
        {
            // in this case only look at potentially *interesting* units rather than huge swaths of nearby
            // units...including target, threat, players, and leader's target
    ...
    {
        findObjects(_Universe->activeStarSystem()->collidemap[Unit::UNIT_ONLY], parent->location[Unit::UNIT_ONLY],
                    &unitLocator);
    }
...
bool FireAt::ShouldFire(Unit *targ, bool &missilelock)
{
    ...
    static float fireangle_minagg =
        (float)cos(M_PI *
                   XMLSupport::parse_float(vs_config->getVariable("AI", "Firing", "MaximumFiringAngle.minagg",
                                                                  "10")) /
                   180.); // Roughly 10 degrees
    static float fireangle_maxagg =
        (float)cos(M_PI *
                   XMLSupport::parse_float(vs_config->getVariable("AI", "Firing", "MaximumFiringAngle.maxagg",
                                                                  "18")) /
    ...
    bool retval =
        ((dist < firewhen) && ((angle > fangle) || (temp && (angle > temp)) || (missilelock && (angle > 0)))) &&
        !isjumppoint;
    if (retval)
    {
...
void FireAt::PossiblySwitchTarget(bool unused)
{
    static float targettime =
        XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "TimeUntilSwitch", "20"));
...
void FireAt::Execute()
{
    ...
    {
        static float cont_update_time =
            XMLSupport::parse_float(vs_config->getVariable("AI", "ContrabandUpdateTime", "1"));
        if (rand() < RAND_MAX * SIMULATION_ATOM / cont_update_time)
        {
            UpdateContrabandSearch();
        }
        static float cont_initiate_time =
            XMLSupport::parse_float(vs_config->getVariable("AI", "CommInitiateTime", "300"));
