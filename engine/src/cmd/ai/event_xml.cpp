...
namespace AIEvents
{
AIEvresult::AIEvresult(int32_t type, const float min, const float max, float time_to_finish,
                       float time_until_interrupts, float priority, const std::string &ai_script)
{
    ...
    if (!validateHardCodedScript(this->script))
    {
        ...
        printf("SERIOUS WARNING in AI script: no fast method to perform %s when type %d is at least %f and at most %f "
               "with priority %f for %f time\n",
               this->script.c_str(), type, min, max, priority, time_to_finish);
    }
}
...
const XMLSupport::EnumMap::Pair AIattribute_names[] = {
    EnumMap::Pair("UNKNOWN", AIUNKNOWN),   EnumMap::Pair("min", AIMIN),
    EnumMap::Pair("max", AIMAX),           EnumMap::Pair("not", AINOT),
    EnumMap::Pair("Script", AISCRIPT),     EnumMap::Pair("time", TIMEIT),
    EnumMap::Pair("obedience", OBEDIENCE), EnumMap::Pair("timetointerrupt", TIMETOINTERRUPT),
...
void LoadAI(const char *filename, ElemAttrMap &result, const string &faction)
{
    ...
    static float cfg_obedience =
        XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "obedience", ".99"));
