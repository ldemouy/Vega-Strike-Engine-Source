...
class GameMissile : public GameUnit<Missile>
{
  protected:
    ...
    GameMissile(const char *filename, int faction, const string &modifications, const float damage, float phasedamage,
  ...
  public:
                                ...
                                const Vector &CumulativeVelocity, bool ResolveLast, UnitCollection *uc = nullptr)
    {
        ...
        if (retarget && targ == nullptr)
            Target(nullptr);
        GameUnit<Missile>::UpdatePhysics2(trans, old_physical_state, accel, difficulty, transmat, CumulativeVelocity,
                                          ResolveLast, uc);
    }
