...
const EnumMap::Pair element_names[] = {
    ...
    EnumMap::Pair("Beam", BEAM),          EnumMap::Pair("Ball", BALL),       EnumMap::Pair("Bolt", BOLT),
    EnumMap::Pair("Missile", PROJECTILE), EnumMap::Pair("Weapons", WEAPONS), EnumMap::Pair("Appearance", APPEARANCE),
    EnumMap::Pair("Energy", ENERGY),      EnumMap::Pair("Damage", DAMAGE),   EnumMap::Pair("Distance", DISTANCE)};
...
#define Gamma_Needed(gamma, count, depth)                                                                              \
    (!((count / (100 * depth * gamma)) % ((6 * (color_step * 100 + depth) / gamma - 1) / 3) - 100))
        ...
        for (iter = attributes.begin(); iter != attributes.end(); iter++)
        {
            ...
            case SPEED:
                ...
                if (tmpweapon.Speed < 1000)
                {
                    tmpweapon.Speed =
                        tmpweapon.Speed *
                        (game_options.gun_speed_adjusted_game_speed ? (1.0 + gun_speed / 1.25) : gun_speed);
                }
                else
                {
                    if (tmpweapon.Speed < 2000)
                    {
                        tmpweapon.Speed =
                            tmpweapon.Speed *
                            (game_options.gun_speed_adjusted_game_speed ? (1.0 + gun_speed / 2.5) : (gun_speed));
                    }
                    else
                    {
                        if (tmpweapon.Speed < 4000)
                            tmpweapon.Speed =
                                tmpweapon.Speed *
                                (game_options.gun_speed_adjusted_game_speed ? (1.0 + gun_speed / 6.0) : (gun_speed));
                        else if (tmpweapon.Speed < 8000)
                            tmpweapon.Speed =
                                tmpweapon.Speed *
                                (game_options.gun_speed_adjusted_game_speed ? (1.0 + gun_speed / 17.0) : (gun_speed));
                    }
                }
                break;
            case PULSESPEED:
            ...
            case RANGE:
                tmpweapon.Range =
                    (game_options.gun_speed_adjusted_game_speed ? (1.0 + gun_speed / 16.0) : (gun_speed)) *
...
weapon_info *getTemplate(const string &kkey)
{
    ...
    if (wi)
    {
        if (!WeaponMeshCache::getCachedMutable(wi->weapon_name))
        {
            ...
            {
                WeaponMeshCache::setCachedMutable(
                    wi->weapon_name, wi->gun = Mesh::LoadMesh(meshname.c_str(), Vector(1, 1, 1), 0, nullptr));
...
float weapon_info::Refire() const
{
    ...
    if (g_game.difficulty > .98 || len < 9 || weapon_name[len - 8] != 'C' || weapon_name[len - 9] != '_' ||
        weapon_name[len - 7] != 'o' || weapon_name[len - 6] != 'm' || weapon_name[len - 5] != 'p' ||
        weapon_name[len - 4] != 'u' || weapon_name[len - 3] != 't' || weapon_name[len - 2] != 'e' ||
        weapon_name[len - 1] != 'r')
        return RefireRate;
    return this->RefireRate * (game_options.refire_difficulty_scaling /
