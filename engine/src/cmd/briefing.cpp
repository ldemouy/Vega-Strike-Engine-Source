...
void Briefing::Render()
{
    ...
    for (uint32_t i = 0; i < starships.size(); i++)
    {
...
void Briefing::RemoveStarship(uint32_t which)
{
    if (which < starships.size() && which >= 0)
    {
        starships[which]->Destroy();
    }
}
void Briefing::EnqueueOrder(uint32_t which, const Vector &dest, float time)
{
    if (which < starships.size() && which >= 0)
    {
...
void Briefing::OverrideOrder(uint32_t which, const Vector &dest, float time)
{
    if (which < starships.size() && which >= 0)
    {
...
void Briefing::SetPosition(uint32_t which, const Vector &Position)
{
    if (which < starships.size() && which >= 0)
    {
...
Vector Briefing::GetPosition(uint32_t which)
{
    if (which < starships.size() && which >= 0)
    {
