...
// X(T) Y(T), where X(0) = X0, and X(1) = X1. <same for y>, hence no need for X calculations concernign Y return. but
// here just for kicks.
//***************************************************
                  ...
                  int32_t tail_mode_start, int32_t tail_mode_end)
{
    ...
    {
        double _t0 = _t;
        yt = int32_t(y0 + (vy0 * _t0) + (((3 * (y1 - y0)) - ((2 * vy0) + vy1)) * pow(_t0, 2)) +
                     (((2 * (y0 - y1)) + (vy0 + vy1)) * pow(_t0, 3)));
    }
    else
    {
        double _t1 = _t;
        yt = int32_t(y1 + (vy1 * _t1) + (((3 * (y2 - y1)) - ((2 * vy1) + vy2)) * pow(_t1, 2)) +
