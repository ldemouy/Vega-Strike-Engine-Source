...
Unit *UnitFactory::createUnit(const char *filename, bool SubUnit, int faction, std::string customizedUnit,
                              Flightgroup *flightgroup, int fg_subnumber, string *netxml, ObjSerial netcreate)
{
     ...
     *                       VSServer->broadcast( netbuf, 0, _Universe->activeStarSystem()->GetZone(), CMD_ENTERCLIENT,
     * true);
     *               }
...
{
    _Universe->netLock(true);
    Nebula *neb = new Nebula(unitfile, SubU, faction, fg, fg_snumber);
    _Universe->netLock(false);
    if (netcreate)
...
{
    _Universe->netLock(true);
    Missile *un = new Missile(filename, faction, modifications, damage, phasedamage, time, radialeffect, radmult,
                              detonation_radius);
    _Universe->netLock(false);
    if (netcreate)
        un->SetSerial(netcreate);
...
Planet *UnitFactory::createPlanet(QVector x, QVector y, float vely, const Vector &rotvel, float pos, float gravity,
                                  ...
                                  const std::string &unitname, BLENDFUNC sr, BLENDFUNC ds, const vector<string> &dest,
                                  const QVector &orbitcent, Unit *parent, const GFXMaterial &ourmat,
                                  const std::vector<GFXLightLocal> &ligh, int faction, string fullname, bool inside_out,
                                  ObjSerial netcreate)
{
    _Universe->netLock(true);
    Planet *p = new Planet(x, y, vely, rotvel, pos, gravity, radius, filename, technique, unitname, dest, orbitcent,
                           parent, faction, fullname, inside_out, ligh.size());
    _Universe->netLock(false);
    if (netcreate)
        p->SetSerial(netcreate);
    /*
     *               // False: Only allow creation through system files?  Doesn't make sense to be able to dynamically
     * generate these.
     ...
     *                       // Send a packet to clients in order to make them create this unit
     *
     *                       addPlanetBuffer( netbuf, x, y, vely, rotvel, pos, gravity, radius, filename, sr, ds, dest,
     * orbitcent, parent, ourmat, ligh, faction, fullname, inside_out, netcreate); endBuffer( netbuf );
     *                       VSServer->broadcast( netbuf, 0, _Universe->activeStarSystem()->GetZone(), CMD_ENTERCLIENT,
     * true);
     *               }
     *               VSServer->invalidateSnapshot();
