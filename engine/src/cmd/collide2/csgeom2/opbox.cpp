...
csBox3::bEdge csBox3::edges[24] = {
    {BOX_CORNER_Xyz, BOX_CORNER_xyz, BOX_SIDE_y, BOX_SIDE_z}, {BOX_CORNER_xyz, BOX_CORNER_Xyz, BOX_SIDE_z, BOX_SIDE_y},
    {BOX_CORNER_xyz, BOX_CORNER_xYz, BOX_SIDE_x, BOX_SIDE_z}, {BOX_CORNER_xYz, BOX_CORNER_xyz, BOX_SIDE_z, BOX_SIDE_x},
    {BOX_CORNER_xYz, BOX_CORNER_XYz, BOX_SIDE_Y, BOX_SIDE_z}, {BOX_CORNER_XYz, BOX_CORNER_xYz, BOX_SIDE_z, BOX_SIDE_Y},
    {BOX_CORNER_XYz, BOX_CORNER_Xyz, BOX_SIDE_X, BOX_SIDE_z}, {BOX_CORNER_Xyz, BOX_CORNER_XYz, BOX_SIDE_z, BOX_SIDE_X},

    {BOX_CORNER_Xyz, BOX_CORNER_XyZ, BOX_SIDE_X, BOX_SIDE_y}, {BOX_CORNER_XyZ, BOX_CORNER_Xyz, BOX_SIDE_y, BOX_SIDE_X},
    {BOX_CORNER_XyZ, BOX_CORNER_XYZ, BOX_SIDE_X, BOX_SIDE_Z}, {BOX_CORNER_XYZ, BOX_CORNER_XyZ, BOX_SIDE_Z, BOX_SIDE_X},
    {BOX_CORNER_XYZ, BOX_CORNER_XYz, BOX_SIDE_X, BOX_SIDE_Y}, {BOX_CORNER_XYz, BOX_CORNER_XYZ, BOX_SIDE_Y, BOX_SIDE_X},
    {BOX_CORNER_XYZ, BOX_CORNER_xYZ, BOX_SIDE_Y, BOX_SIDE_Z}, {BOX_CORNER_xYZ, BOX_CORNER_XYZ, BOX_SIDE_Z, BOX_SIDE_Y},

    {BOX_CORNER_xYZ, BOX_CORNER_xYz, BOX_SIDE_Y, BOX_SIDE_x}, {BOX_CORNER_xYz, BOX_CORNER_xYZ, BOX_SIDE_x, BOX_SIDE_Y},
    {BOX_CORNER_xYZ, BOX_CORNER_xyZ, BOX_SIDE_x, BOX_SIDE_Z}, {BOX_CORNER_xyZ, BOX_CORNER_xYZ, BOX_SIDE_Z, BOX_SIDE_x},
    {BOX_CORNER_xyZ, BOX_CORNER_xyz, BOX_SIDE_x, BOX_SIDE_y}, {BOX_CORNER_xyz, BOX_CORNER_xyZ, BOX_SIDE_y, BOX_SIDE_x},
    {BOX_CORNER_xyZ, BOX_CORNER_XyZ, BOX_SIDE_y, BOX_SIDE_Z}, {BOX_CORNER_XyZ, BOX_CORNER_xyZ, BOX_SIDE_Z, BOX_SIDE_y}};
// Index by BOX_SIDE_? number.
csBox3::bFace csBox3::faces[6] = {{BOX_EDGE_xyz_xyZ, BOX_EDGE_xyZ_xYZ, BOX_EDGE_xYZ_xYz, BOX_EDGE_xYz_xyz},
...
void csBox3::GetConvexOutline(const csVector3 &pos, csVector3 *ar, int32_t &num_array, bool bVisible) const
{
    const csVector3 &bmin = Min();
    const csVector3 &bmax = Max();
    int32_t idx;
...
{
    return csBox3(std::min(box1.minbox.x, box2.minbox.x), std::min(box1.minbox.y, box2.minbox.y),
                  std::min(box1.minbox.z, box2.minbox.z), std::max(box1.maxbox.x, box2.maxbox.x),
                  std::max(box1.maxbox.y, box2.maxbox.y), std::max(box1.maxbox.z, box2.maxbox.z));
}

csBox3 operator+(const csBox3 &box, const csVector3 &point)
{
    return csBox3(std::min(box.minbox.x, point.x), std::min(box.minbox.y, point.y), std::min(box.minbox.z, point.z),
                  std::max(box.maxbox.x, point.x), std::max(box.maxbox.y, point.y), std::max(box.maxbox.z, point.z));
}

csBox3 operator*(const csBox3 &box1, const csBox3 &box2)
{
    return csBox3(std::max(box1.minbox.x, box2.minbox.x), std::max(box1.minbox.y, box2.minbox.y),
                  std::max(box1.minbox.z, box2.minbox.z), std::min(box1.maxbox.x, box2.maxbox.x),
                  std::min(box1.maxbox.y, box2.maxbox.y), std::min(box1.maxbox.z, box2.maxbox.z));
}

bool operator==(const csBox3 &box1, const csBox3 &box2)
{
    return ((box1.minbox.x == box2.minbox.x) && (box1.minbox.y == box2.minbox.y) && (box1.minbox.z == box2.minbox.z) &&
            (box1.maxbox.x == box2.maxbox.x) && (box1.maxbox.y == box2.maxbox.y) && (box1.maxbox.z == box2.maxbox.z));
}
...
bool operator>(const csBox3 &box1, const csBox3 &box2)
{
    return ((box2.minbox.x >= box1.minbox.x) && (box2.minbox.y >= box1.minbox.y) && (box2.minbox.z >= box1.minbox.z) &&
            (box2.maxbox.x <= box1.maxbox.x) && (box2.maxbox.y <= box1.maxbox.y) && (box2.maxbox.z <= box1.maxbox.z));
}

bool operator<(const csVector3 &point, const csBox3 &box)
{
    return ((point.x >= box.minbox.x) && (point.x <= box.maxbox.x) && (point.y >= box.minbox.y) &&
            (point.y <= box.maxbox.y) && (point.z >= box.minbox.z) && (point.z <= box.maxbox.z));
}
