...
public:
	...
	inline void Scale(const Point &p)
	{
		m[0][0] *= p.x;
		m[0][1] *= p.x;
		m[0][2] *= p.x;
	...
	inline void Copy(const Matrix3x3 &source) { CopyMemory(m, source.m, 9 * sizeof(float)); }
	...
	//! Returns a row.
	inline const Point &GetRow(const udword r) const { return *(const Point *)&m[r][0]; }
	//! Returns a row.
	inline Point &GetRow(const udword r) { return *(Point *)&m[r][0]; }
	//! Sets a row.
	...
	inline void SkewSymmetric(const Point &a)
	{
		m[0][0] = 0.0f;
		m[0][1] = -a.z;
		m[0][2] = a.y;
	...
	inline void Neg(const Matrix3x3 &mat)
	{
		m[0][0] = -mat.m[0][0];
		m[0][1] = -mat.m[0][1];
		m[0][2] = -mat.m[0][2];
	...
	//! Add another matrix
	inline void Add(const Matrix3x3 &mat)
	{
		m[0][0] += mat.m[0][0];
		m[0][1] += mat.m[0][1];
		m[0][2] += mat.m[0][2];
	...
	//! Sub another matrix
	inline void Sub(const Matrix3x3 &mat)
	{
		m[0][0] -= mat.m[0][0];
		m[0][1] -= mat.m[0][1];
		m[0][2] -= mat.m[0][2];
	...
	//! Mac
	inline void Mac(const Matrix3x3 &a, const Matrix3x3 &b, float s)
	{
	...
	//! Mac
	inline void Mac(const Matrix3x3 &a, float s)
	{
		m[0][0] += a.m[0][0] * s;
		m[0][1] += a.m[0][1] * s;
		m[0][2] += a.m[0][2] * s;
	...
	//! this = A * s
	inline void Mult(const Matrix3x3 &a, float s)
	{
		m[0][0] = a.m[0][0] * s;
		m[0][1] = a.m[0][1] * s;
		m[0][2] = a.m[0][2] * s;
	...
	inline void Add(const Matrix3x3 &a, const Matrix3x3 &b)
	{
	...
	inline void Sub(const Matrix3x3 &a, const Matrix3x3 &b)
	{
		m[0][0] = a.m[0][0] - b.m[0][0];
		m[0][1] = a.m[0][1] - b.m[0][1];
		m[0][2] = a.m[0][2] - b.m[0][2];
	...
	//! this = a * b
	inline void Mult(const Matrix3x3 &a, const Matrix3x3 &b)
	{
	...
	//! this = transpose(a) * b
	inline void MultAtB(const Matrix3x3 &a, const Matrix3x3 &b)
	{
	...
	//! this = a * transpose(b)
	inline void MultABt(const Matrix3x3 &a, const Matrix3x3 &b)
	{
...
*/
	...
	inline friend Matrix3x3 operator*(float s, const Matrix3x3 &mat)
	{
		return Matrix3x3(
