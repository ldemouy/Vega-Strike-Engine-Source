...
Point &Point::PositiveUnitRandomVector()
{
	...
	x = distribution(engine) * ONE_OVER_RAND_MAX;
	y = distribution(engine) * ONE_OVER_RAND_MAX;
	z = distribution(engine) * ONE_OVER_RAND_MAX;
	Normalize();
	return *this;
}
...
Point &Point::UnitRandomVector()
{
	std::default_random_engine engine;
	std::uniform_real_distribution distribution;
	x = distribution(engine) * ONE_OVER_RAND_MAX - 0.5f;
	y = distribution(engine) * ONE_OVER_RAND_MAX - 0.5f;
	z = distribution(engine) * ONE_OVER_RAND_MAX - 0.5f;
	Normalize();
	return *this;
}
