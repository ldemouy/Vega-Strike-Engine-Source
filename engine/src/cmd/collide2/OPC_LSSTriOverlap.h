				...
				if (fT >= 0.0f)
				{
					...
					else // region 3 (side)
					{
						...
						if (fTmp >= 0.0f)
							...
							fSqrDist = fTmp * (-fTmp / fA00) + fA11 + 2.0f * fB1 + fC;
					}
				}
				else // region 7 (side)
				{
				...
				if (fT >= 0.0)
				{
					if (fT <= fDet) // region 1 (side)
					{
						...
						if (fTmp >= 0.0f)
							...
							fSqrDist = fTmp * (-fTmp / fA11) + fA00 + 2.0f * fB0 + fC;
					}
		...
		else if (fR <= 1.0f)
		{
			if (fS + fT <= 1.0f)
			{
				...
				else // region 0
				{
					...
					fSqrDist = fR * (fA00 * fR + fA01 * fS + fA02 * fT + 2.0f * fB0) + fS * (fA01 * fR + fA11 * fS + fA12 * fT + 2.0f * fB1) + fT * (fA02 * fR + fA12 * fS + fA22 * fT + 2.0f * fB2) + kDiff.SquareMagnitude();
				}
			}
			else
			{
