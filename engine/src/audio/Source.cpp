...
Source::Source(std::shared_ptr<Sound> sound, bool _looping)
    : soundPtr(sound),
      ...
      pfRadiusRatios(1, 1), referenceFreqs(250, 5000),

      gain(1),

      lastKnownPlayingTime(0), lastKnownPlayingTimeTime(getRealTime())
{
    // Some default flags
    setLooping(_looping);
    setRelative(false);
