...
namespace Audio
{
    ...
    namespace __impl
    {
        struct FFData
        {
            ...
            FFData(const std::string &path, VSFileSystem::VSFileType type, Format &fmt, int streamIdx) : pFormatCtx(0),
                                                                                                         pCodecCtx(0),
                                                                                                         pCodec(0),
                                                                                                         pStream(0),
                                                                                                         packetBuffer(0),
                                                                                                         ...
                                                                                                         audioStreamIndex(streamIdx)
            {
                ...
                if ((0 != av_open_input_file(&pFormatCtx, npath.c_str(), NULL, BUFFER_SIZE, NULL)) || (0 > av_find_stream_info(pFormatCtx)))
...
// Dump format info in case we want to know...
...
#endif
                default:
                    throw CodecNotFoundException(errbase + " (unsupported audio format)");
                }
            ...
            bool hasPacket() const
            {
                ...
                return packetBuffer && packetBufferSize && packet.data && packet.size;
#else
...
// Read the next packet, skipping all packets that aren't for this stream
    ...
    void FFStream::seekImpl(double position)
    {
        ...
        if ((targetSample >= ffData->sampleBufferStart) && (targetSample < ffData->sampleBufferStart + ffData->sampleBufferSize))
        {
            // just skip data
