...
void testSimpleSceneWTemplates(const std::string &abtplName, const std::string &beamtplName)
{
    ...
    sm->playSource(beamtpl, "testScene", LVector3(-1, 0, 0), Vector3(0, 0, 1), Vector3(0, 0, 0), 1);
    for (i = 0; i < 40; ++i) // 4s = 40 ticks
    {
        smTick();
    }
...
void testSimpleSceneWDynTemplates()
{
    ...
    std::shared_ptr<SourceTemplate> abtpl(new SourceTemplate("afterburner.wav", VSFileSystem::SoundFile, true));
    std::shared_ptr<SourceTemplate> beamtpl(new SourceTemplate("beam.wav", VSFileSystem::SoundFile, false));
...
class EngParticleListener : public UpdateSourceListener
{
  ...
  public:
    EngParticleListener(const vector<LVector3> &engpaths, double worldsize) : engPaths(engpaths)
    {
    }
    ...
    LVector3 computePosition(int64_t which, double phaseOffset = 0.0) const
    {
        ...
        LVector3 vBase = LVector3(cos(phase + time * speed) * radii, -sin(phase + time * speed) * radii, 0);
    ...
    LVector3 computeVelocity(int64_t which, double phaseOffset = 0.0) const
    {
        ...
        LVector3 dvBase =
            LVector3(-sin(phase + time * speed) * radii * speed, -cos(phase + time * speed) * radii * speed, 0);
...
void testComplexScene(bool doppler)
{
    ...
    std::shared_ptr<SourceTemplate> abtpl =
        TemplateManager::getSingleton()->getSourceTemplate("test.sources:afterburner");
    std::shared_ptr<SourceTemplate> engtpl = TemplateManager::getSingleton()->getSourceTemplate("test.sources:engine");
    std::shared_ptr<SourceTemplate> engexpl =
        TemplateManager::getSingleton()->getSourceTemplate("test.sources:explosion");
    cerr << " ok" << endl;
    ...
    // Fill in engpaths
    std::shared_ptr<SourceListener> englistener(new EngParticleListener(engpaths, worldsize));
    ...
    { // 20s = 200 ticks
        ...
        LVector3 pos = LVector3(cos(phase) * worldsize, -sin(phase) * worldsize, 0);

        LVector3 dpos = LVector3(-sin(phase) * worldsize * speed, -cos(phase) * worldsize * speed, 0);
...
void testStreaming()
{
    ...
    cerr << "  Creating resources" << endl;
    cerr << "    creating sounds..." << flush;
    std::shared_ptr<Sound> music = sm->getRenderer()->getSound("AyMambo.ogg", VSFileSystem::MusicFile, true);
    cerr << " ok" << endl;
    cerr << "    loading sounds..." << flush;
    music->load();
    cerr << " ok" << endl;
...
void testMultiStreaming()
{
    ...
    cerr << "    creating sounds..." << flush;
    std::shared_ptr<Sound> music = sm->getRenderer()->getSound("AyMambo.ogg", VSFileSystem::MusicFile, true);
    std::shared_ptr<Sound> music2 = sm->getRenderer()->getSound("Bliss.ogg", VSFileSystem::MusicFile, true);
    cerr << " ok" << endl;
    cerr << "    loading sounds..." << flush;
    music->load();
    music2->load();
...
void testMultiStreaming2()
{
    ...
    cerr << "  Creating resources" << endl;
    cerr << "    creating sounds..." << flush;
    std::shared_ptr<Sound> music = sm->getRenderer()->getSound("SonOfSandFleaRemix.ogg", VSFileSystem::MusicFile, true);
    std::shared_ptr<Sound> music2 =
        sm->getRenderer()->getSound("SonOfSandFleaRemix.ogg", VSFileSystem::MusicFile, true);
    cerr << " ok" << endl;
    cerr << "    loading sounds..." << flush;
    music->load();
    music2->load();
