...
Codec *CodecRegistry::findByName(const std::string &name) const
{
    ...
    {
        throw CodecNotFoundException(std::string("No codec with name \"") + name + "\" has been registered");
    }
}

template <typename INDEX, typename T> class MappedComparator
{
    const INDEX &_index;
...
Codec *CodecRegistry::findByFile(const std::string &path, VSFileSystem::VSFileType type) const
{
    ...
    {
        ExtensionCodecs::const_iterator eit = extensionCodecs.find(path.substr(sep + 1, std::string::npos));
    ...
    {
        std::sort(candidates.begin(), candidates.end(), MappedComparator<CodecPriority, Codec *>(codecPriority));

        // Why do we need an explicit cast *to* const?
        // See http://www.mpi-inf.mpg.de/~hitoshi/otherprojects/tips/cpp-memo.shtml
        for (auto it = candidates.rbegin(); it != reinterpret_cast<const std::vector<Codec *> &>(candidates).rend();
             ++it)
        {
    ...
    throw CodecNotFoundException(std::string("No registered codec can handle the file \"") + path + "\"");
}
...
CodecRegistration::CodecRegistration(Codec *_codec, int priority) : codec(_codec)
{
