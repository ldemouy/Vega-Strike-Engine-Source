...
Listener::Listener()
    : cosAngleRange(-1, -1), position(0, 0, 0), atDirection(0, 0, -1), upDirection(0, 1, 0), velocity(0, 0, 0),
      radius(1), gain(1), worldToLocal(1) // set to identity, given default settings
{
}
...
{
    return Range<Scalar>(Scalar(acos(cosAngleRange.min)), Scalar(acos(cosAngleRange.max)));
}
...
void Listener::update(int32_t flags)
{
    ...
    {
        worldToLocal = Matrix3(atDirection.cross(upDirection), upDirection, -atDirection).inverse();
        dirty.location = 0;
    }
}
