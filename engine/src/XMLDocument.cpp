...
******************************************************************/
    ...
    XMLElement::XMLElement() : mType(XET_CDATA), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
    {
    }

    XMLElement::XMLElement(const std::string &cdata) : mType(XET_CDATA), mContents(cdata), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
    {
    }

    XMLElement::XMLElement(Type type, const std::string &data) : mType(type), mContents((type == XET_CDATA || type == XET_COMMENT) ? data : std::string()),
                                                                 mAttributes(),
                                                                 mParent(0),
                                                                 mDocument(0),
    ...
    XMLElement::XMLElement(const char *tagName, const char *const *attrValuePairList, unsigned int nAttr) : mType(XET_TAG), mTagName(tagName), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
    {
    ...
    XMLElement::XMLElement(const char *tagName, const char *const *attrValuePairList) : mType(XET_TAG), mTagName(tagName), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
    {
        for (; (attrValuePairList[0] && attrValuePairList[1]); attrValuePairList += 2)
            setAttribute(attrValuePairList[0], attrValuePairList[1]);
    }

    XMLElement::XMLElement(const std::string &tagName, const std::vector<std::string> &attrValuePairList) : mType(XET_TAG), mTagName(tagName), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
    {
    ...
    XMLElement::XMLElement(const std::string &tagName, const std::map<std::string, std::string> &attrValuePairList) : mType(XET_TAG), mTagName(tagName), mAttributes(attrValuePairList), mParent(0), mDocument(0), mIdAttribute(mAttributes.find("id")), mNameAttribute(mAttributes.find("name"))
    {
    }
...
******************************************************************/
    ...
    XMLSerializer::XMLSerializer(const char *encoding, XMLDocument *doc, XMLElement *elem) : options(OPT_DEFAULT), internals(0)
    {
    ...
    {
        return !stream.fail() && _exportXML(stream, ((XMLParserContext *)internals)->document);
    }
