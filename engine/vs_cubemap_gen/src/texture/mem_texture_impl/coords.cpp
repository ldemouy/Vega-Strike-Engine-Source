...
{
    register float X((cc.get_x())) register float Y((cc.get_y())) register float Z((cc.get_z())) register float X2 = X * X;
...
{
    eSide S((snc.get_side().get_eSide())) float U((snc.get_scoords().get_u())) float V((snc.get_scoords().get_v())) switch (S)
    {
