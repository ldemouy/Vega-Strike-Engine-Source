...
// Replaces $(var=default) with the value in variables, or the default (if present) - deletes the reference, if not
// present. Performs recursive replacements - that's useful.
/// FIXME: Make it process #{ifdef()}, #{else} and #{endif} tags, and make it replace $$ with $, and ## with # (and
/// ignore the resulting character, to allow output of tags themselves).
static void replaceVars(string &text, map<string, string> variables)
{
    ...
    while ((where = text.find("$(", ip)) != string::npos)
    {
        if ((when = text.find(')', where + 2)) != string::npos)
        {
            ...
            string var = (whip != string::npos && whip < when) ? text.substr(where + 2, whip - where - 2)
                                                               : text.substr(where + 2, when - where - 2);
...
static string getMaterialHash(const GFXMaterial &mat, bool reflect, bool lighting, bool cullface, float polygon_offset,
                              int blend_src, int blend_dst, float alpha_test)
{
...
static string getMaterialHash(const XML &memfile)
{
    ...
    hash += getMaterialHash(memfile.material, memfile.reflect, memfile.lighting, memfile.cullface,
                            memfile.polygon_offset, memfile.blend_src, memfile.blend_dst, memfile.alphatest);
    hash += "-";
...
struct outputContext
{
    ...
    map<string, pair<string, string>>
        materials;                    // maps a material hash (use getMaterialHash()) to an Ogre Material Name/Script.
    set<string> predefined_materials; // materials that are defined somewhere (as in the .material script already, or in
                                      // another material scripts) - not to be defined again
...
void *Init(const char *inputfile, const char *matfile)
{
    ...
    // Load the mesh
    {
        ...
        ctxt->name = "export-mesh-" + getID();
        ctxt->top = Ogre::MeshManager::getSingleton().createManual(
            ctxt->name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
...
static void SetupColorTplVars(map<string, string> &vars, string mainvar, string attributename, string rvar, string gvar,
                              string bvar, string avar, float r, float g, float b, float a, float defr, float defg,
                              float defb, float defa, bool force_rgba = false)
{
    ...
    vars[mainvar] = ((eqa && !(eqrgb) && !force_rgba) ? attributename + string(" " + rgbs)
                                                      : ((eqrgb && eqa) ? "" : attributename + string(" " + rgbas)));
...
static void SetupBlendTplVars(map<string, string> &vars, int src, int dst)
{
    ...
    if ((src < 0) || (dst < 0) || (size_t(src) > sizeof(blendnames) / sizeof(blendnames[0])) ||
        (size_t(dst) > sizeof(blendnames) / sizeof(blendnames[0])) || ((srcblend = blendnames[src]) == "?") ||
    ...
    if (mit == ctxt->materials.end())
    {
        ...
        int tex[4] = {-1, -1, -1, -1};
        {
            for (vector<textureholder>::size_type i = 0; i < xml.textures.size(); i++)
                if ((xml.textures[i].index >= 0) && (size_t(xml.textures[i].index) < (sizeof(tex) / sizeof(tex[0]))) &&
                    (xml.textures[i].type != UNKNOWN) && (xml.textures[i].name.size() > 0))
        ...
        if ((tplit != templates.end() && !tplit->second.empty()) // First, the appropriate one
            || ((tplit = templates.find("dif+glow+ppl+env")) != templates.end() &&
                !tplit->second.empty()) // Next, try the �bermaterial
        ...
        SetupColorTplVars(vars, "AMBIENT", "ambient", "AR", "AG", "AB", "AA", xml.material.ar, xml.material.ag,
                          xml.material.ab, xml.material.aa, -1.f, -1.f, -1.f, 1.f); //-1.0 so that it's never omitted
        SetupColorTplVars(vars, "AMBIENT_H", "ambient", "AR_H", "AG_H", "AB_H", "AA", xml.material.ar / 2,
                          xml.material.ag / 2, xml.material.ab / 2, xml.material.aa, -1.f, -1.f, -1.f,
                          1.f); //-1.0 so that it's never omitted
        SetupColorTplVars(vars, "AMBIENT_Q", "ambient", "AR_Q", "AG_Q", "AB_Q", "AA", xml.material.ar / 4,
                          xml.material.ag / 4, xml.material.ab / 4, xml.material.aa, -1.f, -1.f, -1.f,
                          1.f); //-1.0 so that it's never omitted
        if (false)
        {
        ...
        {
            SetupColorTplVars(vars, "DIFFUSE", "diffuse", "DR", "DG", "DB", "DA", xml.material.dr, xml.material.dg,
                              xml.material.db, xml.material.da, 1.f, 1.f, 1.f, 1.f);
            SetupColorTplVars(vars, "DIFFUSE_H", "diffuse", "DR_H", "DG_H", "DB_H", "DA", xml.material.dr / 2,
                              xml.material.dg / 2, xml.material.db / 2, xml.material.da, -1.f, -1.f, -1.f, -1.f);
            SetupColorTplVars(vars, "DIFFUSE_Q", "diffuse", "DR_Q", "DG_Q", "DB_Q", "DA", xml.material.dr / 4,
                              xml.material.dg / 4, xml.material.db / 4, xml.material.da, -1.f, -1.f, -1.f, -1.f);
        }
        SetupColorTplVars(vars, "SPECULAR", "specular", "SR", "SG", "SB", "SA", xml.material.sr, xml.material.sg,
                          xml.material.sb, xml.material.sa, 0.f, 0.f, 0.f, 0.f, true);
        if (!vars["SPECULAR"].empty())
            vars["SPECULAR"] += " $(SHININESS)";
        SetupColorTplVars(vars, "SPECULAR_H", "specular", "SR_H", "SG_H", "SB_H", "SA", xml.material.sr / 2,
                          xml.material.sg / 2, xml.material.sb / 2, xml.material.sa, 0.f, 0.f, 0.f, 0.f, true);
        if (!vars["SPECULAR_H"].empty())
            vars["SPECULAR_H"] += " $(SHININESS)";
        SetupColorTplVars(vars, "SPECULAR_Q", "specular", "SR_Q", "SG_Q", "SB_Q", "SA", xml.material.sr / 4,
                          xml.material.sg / 4, xml.material.sb / 4, xml.material.sa, 0.f, 0.f, 0.f, 0.f, true);
        ...
        SetupColorTplVars(vars, "EMISSIVE", "emissive", "ER", "EG", "EB", "EA", xml.material.er, xml.material.eg,
                          xml.material.eb, xml.material.ea, 0.f, 0.f, 0.f, 0.f);
        SetupColorTplVars(vars, "EMISSIVE_H", "emissive", "ER_H", "EG_H", "EB_H", "EA", xml.material.er / 2,
                          xml.material.eg / 2, xml.material.eb / 2, xml.material.ea, 0.f, 0.f, 0.f, 0.f);
        SetupColorTplVars(vars, "EMISSIVE_Q", "emissive", "ER_Q", "EG_Q", "EB_Q", "EA", xml.material.er / 4,
                          xml.material.eg / 4, xml.material.eb / 4, xml.material.ea, 0.f, 0.f, 0.f, 0.f);
        ...
        if (tex[0] != -1)
            vars["TEXTURE_DIF_TYPE"] = "2d";
        if (tex[1] != -1)
            vars["TEXTURE_PPL_TYPE"] = "2d";
        if (tex[2] != -1)
            vars["TEXTURE_DMG_TYPE"] = "2d";
        if (tex[3] != -1)
            vars["TEXTURE_GLOW_TYPE"] = "2d";
        /// FIXME: Propperly handle alphamaps
        vars["BASEPATH"] = ctxt->basepath;
        if (tex[0] != -1)
            vars["TEXTURE_DIF_FILE"] = "$(BASEPATH)$(TEXTURE_DIF_FILENAME)";
        if (tex[1] != -1)
            vars["TEXTURE_PPL_FILE"] = "$(BASEPATH)$(TEXTURE_PPL_FILENAME)";
        if (tex[2] != -1)
            vars["TEXTURE_DMG_FILE"] = "$(BASEPATH)$(TEXTURE_DMG_FILENAME)";
        if (tex[3] != -1)
            vars["TEXTURE_GLOW_FILE"] = "$(BASEPATH)$(TEXTURE_GLOW_FILENAME)";
        if (tex[0] != -1)
            vars["TEXTURE_DIF_FILENAME"] = tostring(xml.textures[tex[0]].name);
        if (tex[1] != -1)
            vars["TEXTURE_PPL_FILENAME"] = tostring(xml.textures[tex[1]].name);
        if (tex[2] != -1)
            vars["TEXTURE_DMG_FILENAME"] = tostring(xml.textures[tex[2]].name);
        if (tex[3] != -1)
            vars["TEXTURE_GLOW_FILENAME"] = tostring(xml.textures[tex[3]].name);
...
static Ogre::VertexData *CreateVertexBuffers(const vector<GFXVertex> &vertices, bool texcoord, bool normals,
                                             Vector3 &min, Vector3 &max, Real &maxsqr,
                                             vector<unsigned int> *indices = 0)
{
    ...
    Ogre::HardwareVertexBufferSharedPtr vbuf = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
        offset, data->vertexCount, Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY, false);
...
static Ogre::HardwareIndexBufferSharedPtr CreateIndexBuffer(const vector<unsigned int> indices)
{
    ...
    Ogre::HardwareIndexBufferSharedPtr buf = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
        use32BitIndexes ? Ogre::HardwareIndexBuffer::IT_32BIT : Ogre::HardwareIndexBuffer::IT_16BIT, indices.size(),
...
static unsigned int deambiguatest(const vector<GFXVertex> &origvertices, vector<GFXVertex> &vertices,
                                  multimap<unsigned int, unsigned int> vertexmap, unsigned int idx, float s, float t)
{
    multimap<unsigned int, unsigned int>::const_iterator it = vertexmap.find(idx);
    while ((it != vertexmap.end() && it->first == idx) &&
...
static void AddMesh(void *outputcontext, const XML &xml, const string &matname, bool texcoord, bool normals)
{
    ...
    if (xml.linestrips.size())
        numprimgroups += int(xml.linestrips.size());
    if (xml.tristrips.size())
        numprimgroups += int(xml.tristrips.size());
    if (xml.trifans.size())
        numprimgroups += int(xml.trifans.size());
    if (xml.quadstrips.size())
        numprimgroups += int(xml.quadstrips.size());
    ...
    if (xml.lines.size())
    {
        ...
        if (!useSharedVert)
            sm->vertexData =
                CreateVertexBuffers(vertices, texcoord, normals, ctxt->min, ctxt->max, ctxt->maxsqr, &indices);
    ...
    if (xml.quads.size())
    {
        ...
        {
            unsigned int idx[] = {idxbase + xml.quads[i].indexref[0], idxbase + xml.quads[i].indexref[1],
                                  idxbase + xml.quads[i].indexref[2], idxbase + xml.quads[i].indexref[3]};
            if (!useSharedVert)
            {
...
#else
    Ogre::VertexDeclaration *newDcl = data->vertexDeclaration->getAutoOrganisedDeclaration(
...
void AutoLOD(void *outputcontext, bool force, int numLod, float reductionFactor, float refDistance)
{
    ...
    if (force || (newMesh->getNumLodLevels() <= 1))
    {
        ...
        newMesh->setLodStrategy(Ogre::LodStrategyManager::getSingletonPtr()->getStrategy("PixelCount"));
#else
...
{
    /// FIXME: Optimize usage of shared vertex buffers (don't do that always)
    struct outputContext *ctxt = (struct outputContext *)outputcontext;
    MeshPtr m = ctxt->top;
    // Shared vertex data
    if (ctxt->sharedvert.size())
        ctxt->top->sharedVertexData = CreateVertexBuffers(ctxt->sharedvert, ctxt->sv_usetex, ctxt->sv_usenorm,
                                                          ctxt->min, ctxt->max, ctxt->maxsqr);
    // Set bounds
