...
public:
            ...
            explicit Link( unsigned int parind, std::string ind, std::string pfile ) : parentindex( parind )
                , pythonfile( pfile )
                , index( ind ) {}
            virtual ~Link() {}
#ifdef BASE_MAKER
...
void InputKeyUp( unsigned char key, int x, int y )
{
    if (is_input) {
        if (key == '\r' || key == '\n') {
            ...
            Output( input_string );
            is_input =
                !( (*input_func)(input_string, input_base_index_dat, input_dat1, input_dat2, input_dat3, input_x, input_y) );
        }
    }
    glutPostRedisplay();
}
...
//}
            ...
            Base::Room::BaseVSSprite *tmp =
                new Base::Room::BaseVSSprite(
                    ("bases/"+Base::CurrentBase->basefile+"/"+input+thetimes[time ? 0 : 3]+".spr").c_str(),
                    string( (char*) dat3 ) );
