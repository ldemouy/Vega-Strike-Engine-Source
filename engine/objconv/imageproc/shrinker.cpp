...
void ModifyImage( unsigned int sizex,
                  ...
                  int nw,
                  int nh,
                  unsigned char out[HEI][WID][3] )
{
    int stride = (img_depth/8)*(img_alpha ? 4 : 3);
    for (unsigned int i = 0; i < nh; ++i)
        for (unsigned int j = 0; j < nw; ++j) {
            const int offset = 0;
            out[i][j][0] = row_pointers[(int) (i*(sizex+offset)/nh)][(int) ( (j*(sizey+offset)/nw) )*stride];
            out[i][j][1] = row_pointers[(int) (i*(sizex+offset)/nh)][(int) ( (j*(sizey+offset)/nw) )*stride+1];
            out[i][j][2] = row_pointers[(int) (i*(sizex+offset)/nh)][(int) ( (j*(sizey+offset)/nw) )*stride+2];
        }
}
