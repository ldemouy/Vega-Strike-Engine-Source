...
void ModifyImage( unsigned int sizex, unsigned int sizey, int img_depth, int img_alpha, unsigned char **row_pointers )
{
    int stride = (img_depth/8)*(img_alpha ? 4 : 3);
    for (unsigned int i = 0; i < sizey; ++i)
        for (unsigned int j = 0; j < sizex; ++j)
            if (row_pointers[i][j*stride+3] == 0) {
                double num_found = 1;
                if (i > 0) {
                    if (row_pointers[i-1][j*stride+3]) {
                        row_pointers[i][j*stride]   = row_pointers[i-1][j*stride];
                        row_pointers[i][j*stride+1] = row_pointers[i-1][j*stride+1];
                        row_pointers[i][j*stride+2] = row_pointers[i-1][j*stride+2];
                    }
                }
                if (j > 0) {
                    if (row_pointers[i][(j-1)*stride+3]) {
                        row_pointers[i][j
                                        *stride] =
                            row_pointers[i][(j-1)*stride]*(1./num_found)+row_pointers[i][j*stride]*(1-1./num_found);
                        row_pointers[i][j*stride
                                        +1] =
                            row_pointers[i][(j-1)*stride+1]*(1./num_found)+row_pointers[i][j*stride+1]*(1-1./num_found);
                        row_pointers[i][j*stride
                                        +2] =
                            row_pointers[i][(j-1)*stride+2]*(1./num_found)+row_pointers[i][j*stride+2]*(1-1./num_found);
                        num_found += 1;
                    }
                }
                if (i < sizey-1) {
                    if (row_pointers[i+1][j*stride+3]) {
                        row_pointers[i][j
                                        *stride] =
                            row_pointers[i+1][j*stride]*(1./num_found)+row_pointers[i][j*stride]*(1-1./num_found);
                        row_pointers[i][j*stride
                                        +1] =
                            row_pointers[i+1][j*stride+1]*(1./num_found)+row_pointers[i][j*stride+1]*(1-1./num_found);
                        row_pointers[i][j*stride
                                        +2] =
                            row_pointers[i+1][j*stride+2]*(1./num_found)+row_pointers[i][j*stride+2]*(1-1./num_found);
                        num_found += 1;
                    }
                }
                if (j < sizex-1) {
